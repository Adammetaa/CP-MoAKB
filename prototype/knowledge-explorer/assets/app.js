...
const projectIntegratedKnowledge = (data, viewId) => {
  ...
  return {
    ...
    applicationContext: view.application_context_view_id ? data.application_context_views?.find((item) => item.id === view.application_context_view_id) || null : null,
    depositionCoverage: view.deposition_coverage_view_id ? data.deposition_coverage_views?.find((item) => item.id === view.deposition_coverage_view_id) || null : null,
    outcomeReview: view.outcome_review_view_id ? data.outcome_review_views?.find((item) => item.id === view.outcome_review_view_id) || null : null,
    crossCaseComparison: view.cross_case_comparison_set_id ? data.cross_case_comparison_sets?.find((item) => item.id === view.cross_case_comparison_set_id) || null : null,
    localPatternAdjudication: view.local_pattern_adjudication_id ? data.local_pattern_adjudications?.find((item) => item.id === view.local_pattern_adjudication_id) || null : null,
    localPatternConflict: view.local_pattern_conflict_adjudication_id ? data.local_pattern_adjudications?.find((item) => item.id === view.local_pattern_conflict_adjudication_id) || null : null,
...
const renderApplicationContext = (view, locale) => {
  const labels = locale === "th" ? { title: "บริบทการใช้งาน", recorded: "สิ่งที่บันทึก", coverage: "หลักฐานการครอบคลุม", limitations: "ข้อจำกัดที่อาจเกี่ยวข้อง", missing: "หลักฐานที่ยังขาด", review: "การทบทวนโดยมนุษย์" } : { title: "Application Context", recorded: "What was recorded?", coverage: "Coverage Evidence", limitations: "Potential Limitations", missing: "Missing Evidence", review: "Human Review" };
  const assertionItems = view.assertions.map((item) => `<li><strong>${escapeHtml(item.subject.replaceAll("_", " "))}</strong>: ${escapeHtml(typeof item.value === "object" ? JSON.stringify(item.value) : item.value)}${item.unit ? ` ${escapeHtml(item.unit)}` : ""}${item.denominator ? `/${escapeHtml(item.denominator)}` : ""}<small>${escapeHtml(item.evidence_state)} Â· ${escapeHtml(item.source)} Â· ${escapeHtml(item.timestamp || "not time-specific")} Â· ${item.direct ? "direct" : "contextual/inferred"}</small><small>${escapeHtml(item.limitations.join("; "))}</small></li>`).join("");
  return `<article class="knowledge-card application-context" style="grid-column: 1 / -1"><div class="section-heading"><div><p class="eyebrow">${escapeHtml(view.id)}</p><h3>${escapeHtml(labels.title)}</h3><p>${escapeHtml(view.subject)} Â· ${escapeHtml(view.application_event.event_type)}</p></div><span class="tag">${escapeHtml(view.states.join(" Â· "))}</span></div><div class="grid-3 corpus-grid"><section><h4>${escapeHtml(labels.recorded)}</h4><ul>${assertionItems}</ul></section><section><h4>${escapeHtml(labels.coverage)}</h4><p><strong>${escapeHtml(view.coverage_evidence.status)}</strong></p><p>${escapeHtml(view.coverage_evidence.limitations.join("; "))}</p><h4>${escapeHtml(labels.limitations)}</h4><ul>${view.potential_limitations.map((item) => `<li>${escapeHtml(item.state)} Â· ${escapeHtml(item.evidence)}<small>${escapeHtml(item.limitations.join("; "))}</small></li>`).join("")}</ul></section><section><h4>${escapeHtml(labels.missing)}</h4><ul>${view.missing_evidence.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul><p><strong>One next evidence action:</strong> ${escapeHtml(view.next_best_evidence.subject)} Â· ${escapeHtml(view.next_best_evidence.completion)}</p><h4>${escapeHtml(labels.review)}</h4><p>Required for unresolved relevance; Human Review cannot create instructions or waive authority.</p></section></div><aside class="boundary-note"><span aria-hidden="true">!</span><div><strong>Recorded application context â‰  confirmed application quality</strong><p>Application Event â‰  execution task Â· potential limitation â‰  instruction Â· failed control â‰  resistance Â· current registration â‰  exact CTU authority. No dose, drone setting, water-volume, timing, re-treatment, product ranking, or automatic Learn output is created.</p></div></aside><small>Provenance: ${escapeHtml(view.application_event.source)} Â· ${escapeHtml(view.application_event.timestamp)} Â· ${escapeHtml(view.application_event.limitations.join("; "))}</small></article>`;
};

const renderDepositionCoverage = (view, locale) => {
  const labels = locale === "th" ? { title: "หลักฐานการตกสะสมและการครอบคลุม", measurements: "การวัด", target: "ตำแหน่งเป้าหมาย", limits: "ข้อจำกัดการสุ่ม", review: "การทบทวนโดยมนุษย์" } : { title: "Deposition & Coverage Evidence", measurements: "Measurements", target: "Target Location", limits: "Sampling Limitations", review: "Human Review" };
  const measurements = view.measurements.map((item) => `<li><strong>${escapeHtml(item.concept)} Â· ${escapeHtml(item.collector_location)}</strong>: ${item.value === null ? "NOT_MEASURED" : `${escapeHtml(item.value)} ${escapeHtml(item.unit)}`}<small>${escapeHtml(item.method)} Â· ${escapeHtml(item.collector_type)} Â· ${escapeHtml(item.collector_orientation)} Â· collectors ${escapeHtml(item.number_of_collectors)}</small><small>${escapeHtml(item.evidence_state)} Â· ${escapeHtml(item.source)} Â· ${escapeHtml(item.timestamp || "no measurement timestamp")} Â· ${escapeHtml(item.limitations.join("; "))}</small></li>`).join("");
  return `<article class="knowledge-card deposition-coverage" style="grid-column: 1 / -1"><div class="section-heading"><div><p class="eyebrow">${escapeHtml(view.id)}</p><h3>${escapeHtml(labels.title)}</h3><p>Application Event: ${escapeHtml(view.application_event_reference)} Â· ${escapeHtml(view.subject)}</p></div><span class="tag">${escapeHtml(view.states.join(" Â· "))}</span></div><div class="grid-3 corpus-grid"><section><h4>${escapeHtml(labels.measurements)}</h4><ul>${measurements}</ul><p><strong>Coverage â‰  Deposition</strong> Â· no common scale or conversion</p></section><section><h4>${escapeHtml(labels.target)}</h4><p><strong>${escapeHtml(view.target_location_binding.target_location)}</strong> Â· ${escapeHtml(view.target_location_binding.measurement_at_target_location)}</p><p>${escapeHtml(view.potential_coverage_gap.statement)}</p><p>Cause established: ${view.potential_coverage_gap.cause_established ? "YES" : "NO"}</p><h4>${escapeHtml(labels.limits)}</h4><ul>${view.sampling_limitations.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></section><section><h4>Threshold Boundary</h4><p>Universal threshold: <strong>NOT AVAILABLE</strong></p><p>No minimum droplets/cmÂ², minimum coverage %, quality score, or pass/fail state is created.</p><p><strong>One next evidence action:</strong> ${escapeHtml(view.next_best_evidence.collector_location)} Â· ${escapeHtml(view.next_best_evidence.completion)}</p><h4>${escapeHtml(labels.review)}</h4><p>${escapeHtml(view.human_review.reasons.join("; "))}</p></section></div><aside class="boundary-note"><span aria-hidden="true">!</span><div><strong>Measured deposition â‰  biological efficacy</strong><p>Low or unknown deposition â‰  causal control failure Â· failed control â‰  resistance Â· measurement â‰  setting recommendation. CP-MoAKB does not instruct increased water, lower flight, nozzle change, or re-spray.</p></div></aside><small>Provenance: ${escapeHtml(view.measurements.flatMap((item) => item.provenance).join("; "))}</small></article>`;
};

const renderOutcomeReview = (view, locale) => {
  const labels = locale === "th" ? { title: "ผลหลังการประยุกต์ใช้", comparison: "การเปรียบเทียบโดยมนุษย์", sampling: "ความสามารถในการเปรียบเทียบการสุ่ม", alternatives: "คำอธิบายทางเลือก", review: "การทบทวนโดยมนุษย์", notConclude: "สิ่งที่ CP-MoAKB ไม่สรุป" } : { title: "Post-Application Outcome", comparison: "Human Comparison", sampling: "Sampling Comparability", alternatives: "Alternative Explanations", review: "Human Review", notConclude: "What CP-MoAKB Does Not Conclude" };
  const effective = ["T0", "T1", "T2"].map((phase) => view.observations.find((item) => item.id === view.effective_observations[phase])).filter(Boolean);
  const observations = effective.map((item) => `<li><strong>${escapeHtml(item.phase)}</strong>: ${escapeHtml(item.value)} ${escapeHtml(item.unit)}/${escapeHtml(item.denominator)}<small>${escapeHtml(item.timestamp)} · ${escapeHtml(item.elapsed_since_application)} · sample ${escapeHtml(item.sample_size)} · ${escapeHtml(item.method)}</small><small>${escapeHtml(item.source)} · explicit submission · ${escapeHtml(item.limitations.join("; "))}</small></li>`).join("");
  const corrections = view.observations.filter((item) => item.supersedes_observation_id).map((item) => `${item.id} supersedes ${item.supersedes_observation_id}`).join("; ");
  return `<article class="knowledge-card outcome-review" style="grid-column: 1 / -1"><div class="section-heading"><div><p class="eyebrow">${escapeHtml(view.id)}</p><h3>${escapeHtml(labels.title)}</h3><p>${escapeHtml(view.subject)} · Application Event ${escapeHtml(view.application_event_reference)}</p></div><span class="tag">${escapeHtml(view.human_comparison.state)} · ${escapeHtml(view.sampling_comparability.state)} · NEEDS_REVIEW</span></div><div class="grid-3 corpus-grid"><section><h4>T0 → Application → T1 → T2</h4><ul>${observations}</ul><small>Correction history: ${escapeHtml(corrections || "none")}</small></section><section><h4>${escapeHtml(labels.comparison)}</h4><p><strong>${escapeHtml(view.human_comparison.state)}</strong></p><p>${escapeHtml(view.human_comparison.statement)}</p><p>Explicit submission: YES</p><h4>${escapeHtml(labels.sampling)}</h4><p><strong>${escapeHtml(view.sampling_comparability.state)}</strong></p><p>${escapeHtml(view.sampling_comparability.limitation)}</p><p>Derived efficacy or control percentage: <strong>NOT CREATED</strong></p></section><section><h4>Application Context / Deposition</h4><p>${escapeHtml(view.application_context_summary.method)} · ${escapeHtml(view.application_context_summary.water_volume)} · ${escapeHtml(view.application_context_summary.canopy)}</p><p>Upper canopy: ${escapeHtml(view.deposition_summary.upper_canopy)} · Plant base: <strong>${escapeHtml(view.deposition_summary.plant_base)}</strong></p><h4>${escapeHtml(labels.alternatives)}</h4><p>${escapeHtml(view.alternative_explanations.join("; "))}</p><h4>${escapeHtml(labels.review)}</h4><p>${escapeHtml(view.human_review.reasons.join("; "))}</p></section></div><aside class="boundary-note"><span aria-hidden="true">!</span><div><strong>${escapeHtml(labels.notConclude)}</strong><p>Observed improvement ≠ product efficacy · temporal order ≠ causality · poor outcome ≠ product failure · no improvement ≠ resistance · failed control ≠ resistance. No score, ranking, recommendation, re-spray, or automatic Learn output is created.</p></div></aside><small>Provenance: ${escapeHtml(effective.flatMap((item) => item.provenance).join("; "))}</small></article>`;
};

const renderCrossCaseComparison = (view, locale) => {
  const labels = locale === "th" ? { title: "การเปรียบเทียบข้ามเคส", question: "คำถามการทบทวน", cases: "เคสที่พิจารณา", dimensions: "มิติการเปรียบเทียบ", review: "การทบทวนโดยมนุษย์", pattern: "ตัวเลือกรูปแบบหลักฐานท้องถิ่น", excluded: "เคสที่แยกออก", notConclude: "สิ่งที่ CP-MoAKB ไม่สรุป" } : { title: "Cross-Case Comparability", question: "Review Question", cases: "Cases Considered", dimensions: "Comparison Dimensions", review: "Human Review", pattern: "Local Pattern Candidate", excluded: "Cases Excluded", notConclude: "What CP-MoAKB Does Not Conclude" };
  const cases = view.cases_considered.map((item) => `<li><strong>${escapeHtml(item.id)}</strong> · ${escapeHtml(item.human_comparison)}<small>T0 ${escapeHtml(item.outcomes.T0)} · T1 ${escapeHtml(item.outcomes.T1)} · T2 ${escapeHtml(item.outcomes.T2)}</small><small>${escapeHtml(item.application_method)} · ${escapeHtml(item.crop_stage)} · ${escapeHtml(item.water_volume)} · ${escapeHtml(item.deposition.plant_base)}</small></li>`).join("");
  const dimensions = view.dimension_findings.map((item) => `<li><strong>${escapeHtml(item.id.replaceAll("_", " "))}</strong>: ${escapeHtml(item.state)}<small>${escapeHtml(item.reason)}</small></li>`).join("");
  const excluded = view.human_review.excluded_cases.map((item) => `<li>${escapeHtml(item.case_id)}<small>${escapeHtml(item.reason)}</small></li>`).join("");
  return `<article class="knowledge-card cross-case-comparison" style="grid-column: 1 / -1"><div class="section-heading"><div><p class="eyebrow">${escapeHtml(view.id)}</p><h3>${escapeHtml(labels.title)}</h3><p><strong>${escapeHtml(labels.question)}:</strong> ${escapeHtml(view.review_question)}</p></div><span class="tag">${escapeHtml(view.overall_comparability)} · ${escapeHtml(view.granularity)}</span></div><div class="grid-3 corpus-grid"><section><h4>${escapeHtml(labels.cases)}</h4><ul>${cases}</ul><h4>${escapeHtml(labels.excluded)}</h4><ul>${excluded}</ul><p>Case records remain independent; no synthetic average Case is created.</p></section><section><h4>${escapeHtml(labels.dimensions)}</h4><ul>${dimensions}</ul><p>Hidden numerical comparability score: <strong>NOT CREATED</strong></p></section><section><h4>${escapeHtml(labels.review)}</h4><p><strong>${escapeHtml(view.human_review.decision)}</strong> · explicit submission</p><p>Included: ${escapeHtml(view.human_review.included_case_ids.join("; "))}</p><h4>${escapeHtml(labels.pattern)}</h4><p><strong>${escapeHtml(view.local_pattern_candidate.state)}</strong> · ${escapeHtml(view.local_pattern_candidate.evidence_class)}</p><p>${escapeHtml(view.local_pattern_candidate.observed_descriptive_pattern)}</p><p>Canonical knowledge: <strong>NO</strong></p></section></div><aside class="boundary-note"><span aria-hidden="true">!</span><div><strong>${escapeHtml(labels.notConclude)}</strong><p>No pooled efficacy, performance score, probability of success, product ranking, resistance claim, recommendation, authority waiver, manufacturer-claim validation, or automatic Learn promotion is created. Similar recorded direction ≠ local product efficacy.</p></div></aside><small>Provenance: ${escapeHtml(view.cases_considered.flatMap((item) => item.provenance).join("; "))}</small></article>`;
};

const renderLocalPatternAdjudication = (view, conflict, locale) => {
  const labels = locale === "th" ? { title: "การพิจารณารูปแบบหลักฐานท้องถิ่น", candidate: "ตัวเลือกรูปแบบ", sufficiency: "ความเพียงพอของหลักฐาน", method: "ขอบเขตวิธีทางสถิติ", review: "การพิจารณาโดยมนุษย์", missing: "หลักฐานที่ยังขาด", conflict: "ส่วนหลักฐานขัดแย้ง", notConclude: "สิ่งที่ CP-MoAKB ไม่สรุป" } : { title: "Local Pattern Adjudication", candidate: "Pattern Candidate", sufficiency: "Evidence Sufficiency", method: "Statistical Method Boundary", review: "Human Adjudication", missing: "Missing Evidence", conflict: "Conflict Representative Slice", notConclude: "What CP-MoAKB Does Not Conclude" };
  const dimensions = view.evidence_dimensions.map((item) => `<li><strong>${escapeHtml(item.id.replaceAll("_", " "))}</strong>: ${escapeHtml(item.state)}<small>${escapeHtml(item.reason)}</small></li>`).join("");
  const conflictText = conflict ? `<h4>${escapeHtml(labels.conflict)}</h4><p><strong>${escapeHtml(conflict.adjudication_state)}</strong> · ${escapeHtml(conflict.pattern_candidate.descriptive_pattern)}</p><p>${escapeHtml(conflict.pattern_splitting.groups.map((item) => `${item.id}: ${item.case_ids.join(", ")}`).join("; "))}</p><p>Original candidate retained: YES</p>` : "";
  return `<article class="knowledge-card local-pattern-adjudication" style="grid-column: 1 / -1"><div class="section-heading"><div><p class="eyebrow">${escapeHtml(view.id)}</p><h3>${escapeHtml(labels.title)}</h3><p><strong>${escapeHtml(labels.candidate)}:</strong> ${escapeHtml(view.pattern_candidate.id)} · ${escapeHtml(view.pattern_candidate.granularity)}</p><p>${escapeHtml(view.pattern_candidate.review_question)}</p></div><span class="tag">${escapeHtml(view.adjudication_state)}</span></div><div class="grid-3 corpus-grid"><section><h4>Cases</h4><p>Considered ${escapeHtml(view.case_counts.total_considered)} · Included ${escapeHtml(view.case_counts.included)} · Excluded ${escapeHtml(view.case_counts.excluded)} · Universal minimum: NOT DEFINED</p><p>${escapeHtml(view.pattern_candidate.descriptive_pattern)}</p><h4>${escapeHtml(labels.missing)}</h4><ul>${view.missing_evidence.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul><p><strong>One next evidence request:</strong> ${escapeHtml(view.evidence_request.subject)} · ${escapeHtml(view.evidence_request.completion)}</p></section><section><h4>${escapeHtml(labels.sufficiency)}</h4><ul>${dimensions}</ul><p>Hidden sufficiency score: <strong>NOT CREATED</strong></p></section><section><h4>${escapeHtml(labels.method)}</h4><p>Descriptive review: <strong>${escapeHtml(view.statistical_method.descriptive_review)}</strong></p><p>Stronger inference: <strong>${escapeHtml(view.statistical_method.stronger_inference)}</strong></p><p>Computations executed: NONE</p><h4>${escapeHtml(labels.review)}</h4><p><strong>${escapeHtml(view.human_adjudication.state)}</strong> · ${escapeHtml(view.human_adjudication.decision)}</p><p>${escapeHtml(view.human_adjudication.reviewer_role)} · ${escapeHtml(view.human_adjudication.reviewer_scope)}</p><p>${escapeHtml(view.human_adjudication.rationale)}</p>${conflictText}</section></div><aside class="boundary-note"><span aria-hidden="true">!</span><div><strong>${escapeHtml(labels.notConclude)}</strong><p>Sufficient for further review ≠ product efficacy · repeated poor outcomes ≠ resistance · reviewer role ≠ authority. No product ranking, recommendation, legal authorization, manufacturer-claim validation, scientific-authority replacement, canonical promotion, or automatic Learn output is created.</p></div></aside><small>Provenance: ${escapeHtml(view.pattern_candidate.provenance.concat(view.id, view.human_adjudication.reviewer).join("; "))}</small></article>`;
};
...
const renderIntegratedKnowledge = () => {
  ...
  if (projection.applicationContext) {
    const applicationHtml = renderApplicationContext(projection.applicationContext, language).replaceAll("\u00c2\u00b7", "\u00b7").replaceAll("\u00e2\u2030\u00a0", "\u2260");
    target.querySelector(".grid-3.corpus-grid")?.insertAdjacentHTML("beforeend", applicationHtml);
  }
  if (projection.depositionCoverage) {
    const depositionHtml = renderDepositionCoverage(projection.depositionCoverage, language).replaceAll("\u00c2\u00b7", "\u00b7").replaceAll("\u00e2\u2030\u00a0", "\u2260");
    target.querySelector(".grid-3.corpus-grid")?.insertAdjacentHTML("beforeend", depositionHtml);
  }
  if (projection.outcomeReview) {
    const outcomeHtml = renderOutcomeReview(projection.outcomeReview, language).replaceAll("\u00c2\u00b7", "\u00b7").replaceAll("\u00e2\u2030\u00a0", "\u2260");
    target.querySelector(".grid-3.corpus-grid")?.insertAdjacentHTML("beforeend", outcomeHtml);
  }
  if (projection.crossCaseComparison) {
    const crossCaseHtml = renderCrossCaseComparison(projection.crossCaseComparison, language).replaceAll("\u00c2\u00b7", "\u00b7").replaceAll("\u00e2\u2030\u00a0", "\u2260");
    target.querySelector(".grid-3.corpus-grid")?.insertAdjacentHTML("beforeend", crossCaseHtml);
  }
  if (projection.localPatternAdjudication) {
    const adjudicationHtml = renderLocalPatternAdjudication(projection.localPatternAdjudication, projection.localPatternConflict, language).replaceAll("\u00c2\u00b7", "\u00b7").replaceAll("\u00e2\u2030\u00a0", "\u2260");
    target.querySelector(".grid-3.corpus-grid")?.insertAdjacentHTML("beforeend", adjudicationHtml);
  }
};
