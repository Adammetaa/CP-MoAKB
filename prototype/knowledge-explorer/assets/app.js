...
const renderIntegratedKnowledge = () => {
  ...
  target.innerHTML = `<div class="section-heading"><div><p class="eyebrow">${escapeHtml(projection.id)}</p><h2>${escapeHtml(projection.title)}</h2></div><span class="tag tag-real">${escapeHtml(projection.managementOptionLink)}</span></div><div class="grid-3 corpus-grid">${assertionList(labels.observed, projection.observedInCase)}${assertionList(labels.scientific, projection.generalKnowledge)}${management}${assertionList(labels.moa, projection.moaKnowledge)}${assertionList(labels.regulatory, projection.regulatoryKnowledge)}${regulatoryBinding}<article class="knowledge-card"><h3>${escapeHtml(labels.products)}</h3><ul>${products}</ul><p><strong>Product information ≠ recommendation</strong></p></article><article class="knowledge-card"><h3>${escapeHtml(labels.gaps)}</h3><ul>${gaps}</ul><p>Human Review: ${projection.humanReviewRequired ? "REQUIRED" : "NOT REQUIRED"}</p></article></div><div class="detail-grid"><div><h3>${escapeHtml(labels.provenance)}</h3><ul>${provenance}</ul></div><aside class="boundary-note"><span aria-hidden="true">!</span><div><strong>Case Evidence ≠ Canonical Knowledge</strong><p>Manufacturer claim ≠ Regulatory Authority · Registration identity ≠ crop-target-use authority · approved rate fact ≠ Case recommendation · CHEMICAL_REVIEW ≠ product selection.</p></div></aside></div>`;
};
