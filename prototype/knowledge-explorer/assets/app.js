...
const renderIntegratedKnowledge = () => {
  ...
  const assertionList = (title, items) => `<article class="knowledge-card"><h3>${escapeHtml(title)}</h3>${items.length ? `<ul>${items.map((item) => `<li>${escapeHtml(item.statement)}${item.details ? `<small>${escapeHtml(`${item.details.observation_timestamp} · ${item.details.burden} ${item.details.unit} · denominator: ${item.details.sampling_denominator} · ${item.details.activity_context} · ${item.details.historical_context} · ${item.details.limitations.join("; ")}`)}</small>` : ""}<small>${escapeHtml(item.provenance.sourceClass)} · ${escapeHtml(item.provenance.sourceId)} · ${escapeHtml(item.provenance.locator)}</small></li>`).join("")}</ul>` : `<p>${escapeHtml(language === "th" ? "ยังไม่มีความสัมพันธ์ที่ยืนยันแล้ว" : "No established relationship")}</p>`}</article>`;
