...
export class PilotStore {
  ...
  summary() {
    ...
    return { workspaces: rows.length, ...totals, feedback, storage_mode:"LOCAL_SQLITE", storage_path:this.dbPath, last_export_at: meta.last_export_at ?? null, last_backup_at: meta.last_backup_at ?? null };
  }
  addFeedback(userId, { route, subject_id = null, rating, note = null }) {
    safeId(userId, "user_id");
    ...
    this.db.prepare("INSERT INTO pilot_feedback(feedback_id,user_id,route,subject_id,rating,note,created_at) VALUES(?,?,?,?,?,?,?)").run(feedback_id, userId, route, subject_id, rating, note?.trim() || null, created_at);
    return { feedback_id, rating, created_at };
  }
  async exportAll() {
    ...
    const feedback = this.db.prepare("SELECT * FROM pilot_feedback ORDER BY created_at").all(), feedbackKeys = ["feedback_id","user_id","route","subject_id","rating","note","created_at"], escapeFeedback = (value) => `"${String(value ?? "").replaceAll('"', '""')}"`;
