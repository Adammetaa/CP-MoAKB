(() => {
  ...
  function evaluate(caseEvidence = {}) {
    ...
    const candidates = managementReview && previousTreatmentKnown ? products.map((product) => {
      ...
      return {
        ...
        regulatoryCtuState: "REGULATORY_CTU_CONFIRMATION_PENDING",
        optionState: registration.state === "CURRENT_RECORD_SUPPORTED" ? "PROVISIONAL_PRODUCT_OPTION" : "BLOCKED_BY_AUTHORITY",
        suitability: "MANUFACTURER_USE_SUPPORTED",
        limitations: [
          `registration history resolves to ${registration.state}`,
    ...
    return {
      ...
      evidenceRequest: managementReview ? { type: "NEED_APPROVED_LABEL", text: "Provide the current official approved label/certificate showing registration number, rice, brown planthopper, rate, timing and cautions." } : null,
  ...
  function render(result) {
    ...
    const cards = result.options.map((option) => `<article class="chemical-option-card" data-option-state="${option.optionState}"><p class="eyebrow">ตัวเลือกสำหรับตรวจทาน · ${option.optionState}</p><h3>${esc(option.product)}</h3><dl><dt>สารสำคัญ / สูตร</dt><dd>${esc(option.activeIngredient)} · ${esc(option.formulation)}</dd><dt>MoA</dt><dd>${esc(option.moa)}</dd><dt>ทะเบียน</dt><dd>${esc(option.registrationNumber)} · ${esc(option.registrationState)}</dd><dt>อัตราที่แหล่งข้อมูลรองรับ</dt><dd>${esc(option.rate)} — ${esc(option.rateBasis)}</dd><dt>ช่วงใช้</dt><dd>ยังไม่มีหลักฐานที่กำกับไว้เพียงพอ</dd><dt>เหตุผลที่เข้ารายการ</dt><dd>ข้าว + เพลี้ยกระโดดสีน้ำตาล + current activity + ≥10 ตัว/ต้น; ใช้เพื่อ review ไม่ใช่ recommendation</dd></dl><details><summary>หลักฐานและข้อจำกัด</summary><p>${option.limitations.map(esc).join(" · ")}</p><ul>${option.sources.map((source) => `<li>${esc(source.authorityClass)} · ${esc(source.id)} · ${esc(source.locator)} · verified ${esc(source.verifiedAt)}</li>`).join("")}</ul></details></article>`).join("");
    return `<section class="chemical-slice" data-chemical-slice><h2>ผล Chemical Management Review</h2><p><strong>Key A: PASS</strong> · Management review justified</p><p><strong>Key B: NOT PASS</strong> · REGULATORY_CTU_CONFIRMATION_PENDING</p><p>พบ ${result.options.length} ผลิตภัณฑ์ที่มีหลักฐานพอสำหรับการตรวจทานแบบมีขอบเขต ไม่ใช่คำแนะนำหรือคำสั่งใช้</p>${cards}<aside class="evidence-request"><strong>NEED_APPROVED_LABEL</strong><p>${esc(result.evidenceRequest.text)}</p></aside><p class="boundary-copy">Product candidate ≠ eligible option ≠ recommendation ≠ execution · Manufacturer guidance ≠ Thai regulatory CTU authority · ไม่มีการจัดอันดับผู้ผลิต</p></section>`;
  }

  window.SPChemicalSlice = Object.freeze({ products, resolveRegistrationHistory, evaluate, render });
})();
