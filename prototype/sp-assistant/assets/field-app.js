...
function brandMarkup(compact = false) {
  const destination=governedRuntime.captureOnly()?"fields":"home";
...
function bottomNavigation(active) {
  const items = governedRuntime.captureOnly()?[["fields", "▦", "แปลง"], ["free-chat", "▢", "แชท"], ["profile", "♙", "โปรไฟล์"]]:[["home", "⌂", "หน้าหลัก"], ["fields", "▦", "แปลง"], ["free-chat", "▢", "แชท"], ["learn", "▤", "เรียนรู้"], ["profile", "♙", "โปรไฟล์"]];
...
function renderFieldDetail() {
  ...
  root.innerHTML = `${appHeader({ back: "home", bell: true })}<main class="app-main field-workspace-main"><section class="field-workspace-header"><div class="field-context-copy"><p class="eyebrow">แปลงที่ระบบยืนยัน · ${escapeHtml(field.field_id)}</p><h1>${escapeHtml(field.name)} <span>★</span></h1><div class="field-context-facts"><span>🌾 ${escapeHtml(field.variety)}</span><span>▦ ${thaiDate(field.planting_date ?? field.expected_planting_date)}</span><span>🍃 ${ageLabel}</span></div><div class="cmp-primary"><small>ระยะ CMP ปัจจุบัน</small><strong>${escapeHtml(field.current_cmp_stage.label)}</strong><span>${escapeHtml(field.current_crop_stage.label)} · ${escapeHtml(field.stage_provenance)}</span></div></div><div class="workspace-map-weather"><div class="workspace-mini-map" data-field-satellite-preview>${polygonPreview(field)}<span>⌖</span><small>${formatArea(field.area)}</small></div>${weatherMarkup()}</div></section><section class="workspace-guidance"><header class="section-heading"><div><p class="eyebrow">สิ่งที่ควรตรวจวันนี้</p><h2>รายการจากระบบที่กำกับไว้</h2></div><span class="pending-count">${pending.length} รายการจาก server</span></header><div class="guidance-list">${pending.map((item) => `<button class="guidance-row ${item.status.toLowerCase()}" type="button" data-guidance-start="${item.guidance_item_id}"><span class="guidance-domain">${domainIcon(item.domain)}</span><span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.short_instruction)}</small><em>${escapeHtml(item.reason)}</em></span><b>เริ่ม ›</b></button>`).join("")||`<div class="empty-inline">เริ่มเคสแล้วระบบจะประเมิน Step C และเสนอ Step D หนึ่งอย่าง</div>`}</div><div class="workspace-actions"><button class="primary-action" type="button" data-action="start-full-inspection">⌕ เริ่มตรวจสุขภาพแปลง</button><button class="secondary-action" type="button" data-action="open-investigation-capture">＋ บันทึกหลักฐานที่พบ</button><button class="secondary-action" type="button" data-action="open-free-chat">▢ ถาม SP Assistant</button></div></section><section class="tip-card"><span>💡</span><div><strong>หลักการบันทึก</strong><p>บันทึกสิ่งที่เห็นตามจริง ระบบไม่ใช้ข้อมูลใน browser เป็นคำตอบทางวิทยาศาสตร์</p></div></section><section class="case-history"><header class="section-heading"><div><p class="eyebrow">SERVER CASE HISTORY</p><h2>ประวัติจะสร้างใหม่จาก server เมื่อเปิดเคสหรือบทสนทนา</h2></div></header></section></main>${bottomNavigation("fields")}`;
...
async function startInspection(guidanceItemId=null) {
  ...
  governedCase=await governedRuntime.startCase(field,guidanceItemId?`เริ่มตรวจตาม Guidance ${guidanceItemId}`:"เริ่มตรวจสุขภาพแปลง");activeCaseId=governedCase.scope.case_id;activeConversationId=null;route="inspection";renderInspection();
}

function renderInspection() {
  const field = selectedField(); if (!field || !activeCaseId) { route = "field-detail"; render(); return; }
  const presentation=casePresentation(governedCase),observations=governedCase?.bundle?.observations??[],response=governedCase?.lastResponse;
  root.innerHTML = `${appHeader({ back:"field-detail" })}<main class="inspection-main"><section class="inspection-context"><div><p class="eyebrow">กำลังตรวจ · SERVER GOVERNED CASE</p><h1>${escapeHtml(field.name)}</h1><span>${escapeHtml(field.current_cmp_stage?.label??"")} · ${escapeHtml(activeCaseId)}</span></div><button class="finish-inspection" type="button" data-action="finish-inspection">ดูสรุป</button></section><section class="inspection-thread">${observations.map((item)=>`<article class="inspection-answer"><small>บันทึกแล้ว · ${escapeHtml(item.observation_id)}</small><p>${escapeHtml(item.note??"ข้อสังเกตจากแปลง")}</p></article>`).join("")}${presentation.visual.map((item)=>`<article class="photo-evidence-message"><span>▧</span><div><strong>${escapeHtml(item.level)}</strong><p>${escapeHtml(item.boundary)}</p><small>${item.quality?"มีสถานะคุณภาพ/การมองเห็นแล้ว":"รอการทบทวนคุณภาพและสิ่งที่มองเห็น"}</small>${item.level==="VE0_RAW_IMAGE_ONLY"?`<button type="button" data-manual-photo-review="${escapeHtml(item.id)}">เริ่มการทบทวนภาพด้วยคน</button>`:""}</div></article>`).join("")}${response?.message?`<article class="current-question"><div class="assistant-badge">SP</div><div><p>${escapeHtml(response.message)}</p></div></article>`:""}<article class="current-question"><div class="assistant-badge">SP</div><div><p class="eyebrow">บอกสิ่งที่เห็นตามธรรมชาติ</p><h2>${escapeHtml(presentation.next_action)}</h2><p class="question-help">หนึ่งประโยคอาจบันทึกได้หลายมิติ · ระบบถามเพิ่มไม่เกินหนึ่งคำถามต่อรอบ และภาพไม่ใช่การวินิจฉัย</p></div></article></section><form class="inspection-composer" data-inspection-form><label class="camera-control" aria-label="แนบรูป"><input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" data-inspection-photo><span>▣</span></label><input name="observation" placeholder="เช่น ข้าวเหลืองเป็นหย่อมตรงจุดต่ำ น้ำลึกกว่าข้างๆ" required><button type="submit" aria-label="ส่งข้อสังเกต">↑</button></form></main>${bottomNavigation("fields")}`;
}
...
function renderFreeChat() {
  ...
  root.innerHTML = `${appHeader({ back: "fields" })}<main class="free-chat-main"><section class="chat-field-context"><span>✓ กำลังบันทึก</span><strong>${escapeHtml(field.name)}</strong><small>${escapeHtml(field.current_cmp_stage.label)}${activeCaseId?` · เคส ${escapeHtml(activeCaseId.slice(-8))}`:" · จะเริ่มเคสเมื่อส่งข้อความ"}</small></section><p class="chat-boundary">บอกเฉพาะสิ่งที่เห็นจริงได้ทีละอย่าง ระบบจะถามเพิ่มไม่เกินครั้งละหนึ่งคำถาม</p><section class="free-chat-thread">${messages.length ? messages.map((message) => `<article class="free-message ${message.role.toLowerCase()}"><span>${message.role === "USER" ? "คุณ" : "SP"}</span><div><p>${escapeHtml(message.content)}</p>${message.evidence_id ? `<small>รับรูปแล้ว · ยังไม่ใช่ผลวินิจฉัย</small>` : ""}<time>${new Intl.DateTimeFormat("th-TH", { hour:"2-digit", minute:"2-digit" }).format(new Date(message.created_at))}</time></div></article>`).join("") : `<article class="free-message assistant"><span>SP</span><div><p>วันนี้เห็นอะไรในแปลงนี้ครับ พิมพ์ตามที่เห็น หรือส่งรูปได้เลย</p><small>ถ้ายังไม่รู้ ตอบ “ไม่รู้” หรือ “ไว้ดูทีหลัง” ได้</small></div></article>`}${chatPending ? `<article class="free-message assistant pending"><span>SP</span><div><p>กำลังบันทึก…</p></div></article>` : ""}${chatRetryText ? `<button class="chat-retry" type="button" data-action="retry-chat">ลองส่งอีกครั้ง</button>` : ""}<div class="chat-suggestions"><button type="button" data-chat-prompt="ไม่รู้">ไม่รู้</button><button type="button" data-chat-prompt="ไว้ดูทีหลัง">ไว้ดูทีหลัง</button><button type="button" data-chat-prompt="ผมมีภาพจากแปลง">ส่งภาพจากแปลง</button></div></section><form class="free-chat-composer" data-free-chat-form><label class="camera-control" aria-label="แนบรูป"><input type="file" accept="image/*" capture="environment" data-chat-photo ${chatPending ? "disabled" : ""}><span>▣</span></label><input name="message" placeholder="พิมพ์สิ่งที่เห็น…" required ${chatPending ? "disabled" : ""}><button type="submit" aria-label="ส่งข้อความ" ${chatPending ? "disabled" : ""}>↑</button></form></main>${bottomNavigation("free-chat")}`;
}
...
function renderProfile() {
  ...
  root.innerHTML = `${appHeader({ back: captureOnly?"fields":"home" })}<main class="app-main simple-page"><div class="profile-card"><div class="large-avatar">👨🏽‍🌾</div><p class="eyebrow">${captureOnly?"บัญชีผู้ใช้":"บัญชีต้นแบบ"}</p><h1>${escapeHtml(user.display_name)}</h1><p>${escapeHtml(user.username)}${captureOnly?` · ${roleLabel}`:` · ${escapeHtml(user.role)}`}</p>${captureOnly?`<dl><div><dt>สถานะบัญชี</dt><dd>ยืนยันโดยระบบแล้ว</dd></div></dl>`:`<dl><div><dt>รหัสผู้ใช้</dt><dd>${escapeHtml(user.user_id)}</dd></div><div><dt>โหมดเข้าสู่ระบบ</dt><dd>${escapeHtml(user.session.authentication_mode)}</dd></div></dl><button class="primary-action" type="button" data-action="open-pilot-diagnostics">สถานะ Internal Pilot</button>`}<button class="secondary-action" type="button" data-action="logout">ออกจากระบบ</button></div></main>${bottomNavigation("profile")}`;
}
...
function render() {
  ...
  if (route === "loading") renderLoading(); else if (route === "login") renderLogin(); else if (route === "gps") renderGps(); else if (route === "home") renderHome(); else if (route === "fields") renderFields(); else if (route === "create") renderCreateField(); else if (route === "field-detail") renderFieldDetail(); else if (route === "investigation-capture") renderInvestigationCapture(); else if (route === "inspection") renderInspection(); else if (route === "summary") governedCase?renderGovernedSummary():renderSummary(); else if (route === "free-chat") renderFreeChat(); else if (route === "learn") renderLearn(); else if (route === "profile") renderProfile(); else if (route === "pilot-diagnostics") renderPilotDiagnostics(); else renderError();
...
root.addEventListener("click", async (event) => {
  ...
  const fieldTarget = event.target.closest("[data-field-open]"); if (fieldTarget) { const nextFieldId = fieldTarget.dataset.fieldOpen; if (selectedFieldId !== nextFieldId) weatherState = undefined; selectedFieldId = nextFieldId; fieldService.select_field(selectedFieldId, currentUser().user_id);if(governedRuntime.captureOnly()&&governedRuntime.scopeFor(selectedField())){activeCaseId=null;await resumeFieldConversation();route="free-chat";}else route = "field-detail"; render(); return; }
  ...
  if (action === "open-free-chat") { const field=selectedField();if(!field){route="fields";render();return;}if(!governedRuntime.scopeFor(field)){formError="แปลงนี้ยังเป็นข้อมูลร่างบนอุปกรณ์ บันทึกแปลงบนระบบก่อนเริ่มเคส";renderFieldDetail();return;}activeCaseId=null;await resumeFieldConversation();route="free-chat";render();return; }
  ...
  if (action === "logout") { try{await serverWorkspace.logout();}catch(error){formError=error.message;renderProfile();return;}const state = workspace(); state.active_user_id = null; repository.save(state); selectedFieldId = null;activeCaseId=null;activeConversationId=null;governedRuntime.identity=null;governedRuntime.caseState=null; route = "login"; render(); return; }
...
root.addEventListener("change", async (event) => {
  if (event.target.matches("[data-inspection-photo]")) {
    ...
    try{await governedRuntime.uploadPhoto(file);governedCase=governedRuntime.caseState;notice="รับภาพเป็น Visual Evidence แล้ว · รอคุณภาพ/การมองเห็นและการทบทวนโดยมนุษย์";}catch(error){formError=error.message;}event.target.value="";renderInspection();return;
  }
