...
function companyStageMarkup(stage, { compact = false } = {}) {
  ...
  return `<article class="company-program-card ${compact ? "compact" : ""}"><header><span>🌾</span><div><small>สูตรมาตรฐานบริษัท · ${escapeHtml(stage.stage_id)}</small><h3>${escapeHtml(stage.label)}</h3><p>${escapeHtml(stage.timing)} · ${escapeHtml(stage.objective)}</p></div></header><div class="program-products">${stage.items.map(([name,rate]) => `<div><strong>${escapeHtml(name)}</strong><span>${escapeHtml(rate)}</span></div>`).join("")}</div><details><summary>ดูค่าโดรนและสิ่งที่ต้องตรวจ</summary><dl class="program-drone"><div><dt>XAG P100 / P100 Pro</dt><dd>${escapeHtml(stage.drone.p100)}</dd></div><div><dt>XAG P60</dt><dd>${escapeHtml(stage.drone.p60)}</dd></div><div><dt>ค่าลมในเอกสารต้นทาง</dt><dd>${escapeHtml(stage.drone.source_wind_limit)}</dd></div></dl><ul>${stage.checks.map((check) => `<li>${escapeHtml(check)}</li>`).join("")}</ul><p>อ้างอิง PDF หน้า ${stage.source_pages.join(", ")}</p></details><p class="program-warning">⚠ ค่ามาตรฐานยังไม่ปรับตามแรงลมจริง · ตรวจฉลาก ทะเบียน อากาศ และยืนยันหน้างานก่อนใช้</p></article>`;
}
...
function renderSummary() {
  ...
  loadCompanyProgram(); const companyStage = companyProgram?.stages?.find((stage) => stage.stage_id === field.current_cmp_stage.stage_id);
  ...
  root.querySelector(".application-boundary")?.insertAdjacentHTML("beforebegin", `<section class="company-program-section"><header><div><p class="eyebrow">COMPANY REFERENCE</p><h2>สูตรมาตรฐานบริษัทตามระยะ CMP</h2></div><span>🔒 Internal Pilot</span></header>${companyStageMarkup(companyStage)}</section>`);
}
...
function renderLearn() {
  ...
  root.querySelector(".knowledge-categories")?.insertAdjacentHTML("beforebegin", `<section class="program-library"><header><div><p class="eyebrow">COMPANY RICE PROGRAM</p><h2>สูตรมาตรฐานบริษัทตาม 9 ระยะ CMP</h2><p>เปิดดูผลิตภัณฑ์ อัตราต่อไร่ และค่าโดรนจากแหล่งข้อมูลที่บริษัทอนุญาต</p></div><span>🔒 สำหรับ Internal Pilot</span></header><div class="program-stage-list">${programMarkup}</div><p class="program-data-boundary">ข้อมูลชุดนี้แสดงเฉพาะหลังล็อกอิน และไม่ถูกส่งไปยัง OpenAI</p></section>`);
}
