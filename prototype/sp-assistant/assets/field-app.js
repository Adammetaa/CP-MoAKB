...
function renderProfile() {
  const user = currentUser();
  root.innerHTML = `${appHeader({ back: governedRuntime.captureOnly()?"fields":"home" })}<main class="app-main simple-page"><div class="profile-card"><div class="large-avatar">👨🏽‍🌾</div><p class="eyebrow">บัญชีต้นแบบ</p><h1>${escapeHtml(user.display_name)}</h1><p>${escapeHtml(user.username)} · ${escapeHtml(user.role)}</p><dl><div><dt>รหัสผู้ใช้</dt><dd>${escapeHtml(user.user_id)}</dd></div><div><dt>โหมดเข้าสู่ระบบ</dt><dd>${escapeHtml(user.session.authentication_mode)}</dd></div></dl>${governedRuntime.captureOnly()?"":`<button class="primary-action" type="button" data-action="open-pilot-diagnostics">สถานะ Internal Pilot</button>`}<button class="secondary-action" type="button" data-action="logout">ออกจากระบบ</button></div></main>${bottomNavigation("profile")}`;
}
