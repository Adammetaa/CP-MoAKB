...
export class MapService {
  ...
  calculate_area(polygon) { const points = polygon.coordinates[0]; const referenceLatitude = points.reduce((sum, point) => sum + point[1], 0) / points.length * Math.PI / 180; const projected = points.map(([longitude, latitude]) => ({ x: longitude * 111_320 * Math.cos(referenceLatitude), y: latitude * 110_540 })); let twiceArea = 0; for (let index = 0; index < projected.length - 1; index += 1) twiceArea += projected[index].x * projected[index + 1].y - projected[index + 1].x * projected[index].y; const squareMeters = Math.abs(twiceArea) / 2; return { square_meters: squareMeters, rai: squareMeters / 1600, hectares: squareMeters / 10000, method: "LOCAL_PLANAR_SHOELACE", estimated: true }; }
}
