...
export class UserAttachmentService{
  ...
  get(requesterUserId,attachmentId,{reviewer=false}={}){id(requesterUserId,"requester_user_id");id(attachmentId,"attachment_id");const row=reviewer?this.db.prepare("SELECT * FROM governed_user_attachments WHERE attachment_id=?").get(attachmentId):this.db.prepare("SELECT * FROM governed_user_attachments WHERE attachment_id=? AND owner_user_id=?").get(attachmentId,requesterUserId);if(!row)fail("attachment not found","NOT_FOUND",404);return record(row);}
  list(requesterUserId,scope={}, {reviewer=false}={}){id(requesterUserId,"requester_user_id");const where=[],values=[];if(!reviewer){where.push("owner_user_id=?");values.push(requesterUserId);}for(const key of ["field_id","season_id","case_id","conversation_id"]){if(scope[key]){where.push(`${key}=?`);values.push(id(scope[key],key));}}return this.db.prepare(`SELECT * FROM governed_user_attachments${where.length?` WHERE ${where.join(" AND ")}`:""} ORDER BY created_at,attachment_id`).all(...values).map(record);}
}
