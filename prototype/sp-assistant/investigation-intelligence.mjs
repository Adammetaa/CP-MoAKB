...
import { loadCandidateProvider } from "./candidate-provider.mjs";
...
const MANAGEMENT_SUFFICIENCY_PACKAGE=JSON.parse(readFileSync(new URL("./step-c-management-sufficiency-package.json",import.meta.url),"utf8"));
...
function validatedRule(raw,candidates,sources){
  object(raw,"rule"); allowed(raw,["rule_id","candidate_id","dimension","relation","constraint","reason","source_refs","missing_evidence","next_best_evidence","semantic_basis","negative_feature","inspection_method_appropriate","provider_provenance"],"rule");
  ...
  if(relation==="REQUIRED_BUT_MISSING"){
    ...
    missing={gap_ref:id(raw.missing_evidence.gap_ref,"gap_ref"),concept:text(raw.missing_evidence.concept,"missing concept",200),importance:member(raw.missing_evidence.importance,new Set(["REQUIRED","USEFUL"]),"missing importance"),reason:text(raw.missing_evidence.reason,"missing reason",1_000),rule_ref:ruleId,rule_provenance:raw.provider_provenance??null};
    if(raw.next_best_evidence)next=validateNextRequest(raw.next_best_evidence,candidateId,missing.gap_ref);
  }
  return {rule_id:ruleId,candidate_id:candidateId,dimension,relation,constraint,reason:text(raw.reason,"rule reason",1_000),source_refs:refs,missing,next,semantic_basis:raw.semantic_basis??null,negative_feature:raw.negative_feature??null,inspection_method_appropriate:raw.inspection_method_appropriate===true,provider_provenance:raw.provider_provenance??null};
}
...
function relationEntry({relation,dimension,constraint,reason,ruleRef,sourceRef,ruleProvenance=null}){
  return {evidence_id:sourceRef?.record_id??null,source_type:sourceRef?.record_type??null,relation,dimension,constraint,reason,rule_ref:ruleRef,rule_provenance:ruleProvenance};
}
...
function validateManagementSufficiencyPackage(raw){
  object(raw,"Step C management sufficiency package");object(raw.manifest,"Step C management sufficiency manifest");
  const manifest={provider_id:id(raw.manifest.provider_id,"management sufficiency provider_id"),provider_version:text(raw.manifest.provider_version,"management sufficiency provider_version",100),authority:member(raw.manifest.authority,new Set(["GOVERNED_KNOWLEDGE_RELATIONSHIP"]),"management sufficiency authority"),review_state:member(raw.manifest.review_state,new Set(["DOMAIN_APPROVED"]),"management sufficiency review state"),valid_from:text(raw.manifest.valid_from,"management sufficiency valid_from",100)};
  const sources=(raw.sources??[]).map((source)=>{object(source,"management sufficiency source");return{source_id:text(source.source_id,"management sufficiency source_id",200),repository_path:text(source.repository_path,"management sufficiency repository_path",500),record_ids:(source.record_ids??[]).map((value)=>text(value,"management sufficiency source record",240)),authority:member(source.authority,new Set(["GOVERNED_KNOWLEDGE_RELATIONSHIP"]),"management sufficiency source authority"),review_state:member(source.review_state,new Set(["DOMAIN_APPROVED"]),"management sufficiency source review state")};});
  const rules=(raw.rules??[]).map((rule)=>{object(rule,"management sufficiency rule");const current=(rule.required_current_evidence??[]).map((requirement)=>{object(requirement,"required current evidence");return{evidence_type:id(requirement.evidence_type,"required evidence type"),allowed_evidence_levels:(requirement.allowed_evidence_levels??[]).map((value)=>id(value,"allowed evidence level")),allowed_review_states:(requirement.allowed_review_states??[]).map((value)=>id(value,"allowed evidence review state"))};});return{rule_id:text(rule.rule_id,"management sufficiency rule_id",200),purpose:member(rule.purpose,new Set(["MANAGEMENT_OPTION_REVIEW"]),"management sufficiency purpose"),sufficient_state:member(rule.sufficient_state,new Set(["SUFFICIENT_FOR_MANAGEMENT_OPTION_REVIEW"]),"management sufficient state"),candidate_concept_ids:(rule.candidate_concept_ids??[]).map((value)=>text(value,"candidate concept",200)),crop:id(rule.crop,"management sufficiency crop"),required_finding_role:member(rule.required_finding_role,sets.findingRoles,"required finding role"),allowed_resolution_levels:(rule.allowed_resolution_levels??[]).map((value)=>member(value,sets.resolutionLevels,"allowed resolution level")),allowed_candidate_states:(rule.allowed_candidate_states??[]).map((value)=>member(value,sets.candidateStates,"allowed candidate state")),minimum_supporting_case_evidence:Number(rule.minimum_supporting_case_evidence),required_current_evidence:current,allow_missing_required_evidence:rule.allow_missing_required_evidence===true,allow_contradicting_case_evidence:rule.allow_contradicting_case_evidence===true,source_refs:(rule.source_refs??[]).map((value)=>text(value,"management sufficiency source ref",240)),limitations:(rule.limitations??[]).map((value)=>text(value,"management sufficiency limitation",1_000))};});
  const governedRecords=new Set(sources.flatMap((source)=>source.record_ids));if(!sources.length||!rules.length||rules.some((rule)=>!Number.isInteger(rule.minimum_supporting_case_evidence)||rule.minimum_supporting_case_evidence<1||!rule.source_refs.length||rule.source_refs.some((ref)=>!governedRecords.has(ref))||!rule.limitations.length||!rule.required_current_evidence.length))fail("Step C management sufficiency package is incomplete");
  return{schema_version:text(raw.schema_version,"management sufficiency schema_version",100),manifest,sources,rules};
}

export class GovernedInvestigationRuleProvider extends PersistedCandidateRuleProvider {
  constructor(rulePackage=MANAGEMENT_SUFFICIENCY_PACKAGE){super();this.package=validateManagementSufficiencyPackage(structuredClone(rulePackage));this.contentHash=hash(this.package);this.version=`${this.package.manifest.provider_id}@${this.package.manifest.provider_version}`;this.authority=this.package.manifest.authority;this.reviewState=this.package.manifest.review_state;}
  getRules(){return{rule_version:this.version,rule_authority:this.authority,rule_review_state:this.reviewState,rules:[]};}
  getManifest(){return{...this.package.manifest,content_hash:this.contentHash};}
  adjudicateManagementSufficiency(bundle,assessment,reviewHistory){
    const caseId=bundle?.cases?.length===1?bundle.cases[0].case_id:null,crop=String(bundle?.field_context?.crop??"").toUpperCase(),caseEvidence=new Map((bundle?.evidence??[]).map((item)=>[item.evidence_id,item])),review=[...(reviewHistory??[])].reverse().find((item)=>item.action==="RESOLVE_FINDING");
    for(const rule of this.package.rules){
      const finding=review?.candidate_findings?.find((item)=>item.role===rule.required_finding_role),candidate=(assessment.candidate_assessments??[]).find((item)=>item.candidate_id===finding?.candidate_id),supporting=(candidate?.supporting_evidence??[]).map((item)=>item.evidence_id).filter((ref)=>caseEvidence.has(ref)),contradicting=(candidate?.contradicting_evidence??[]).map((item)=>item.evidence_id).filter(Boolean),missing=(candidate?.missing_evidence??[]).map((item)=>item.gap_ref).filter(Boolean),current=[];
      for(const requirement of rule.required_current_evidence){const match=[...caseEvidence.values()].find((item)=>item.evidence_type===requirement.evidence_type&&requirement.allowed_evidence_levels.includes(item.evidence_level)&&requirement.allowed_review_states.includes(item.review_state));if(match)current.push(match.evidence_id);}
      const checks={same_case:Boolean(caseId&&caseId===assessment.scope?.case_id),crop_matches:crop===rule.crop,candidate_matches:Boolean(candidate&&rule.candidate_concept_ids.includes(candidate.concept_id)),candidate_state_supported:Boolean(candidate&&rule.allowed_candidate_states.includes(candidate.state)),human_review_matches:Boolean(review&&rule.allowed_resolution_levels.includes(review.resolution_level)),supporting_case_evidence_present:supporting.length>=rule.minimum_supporting_case_evidence,current_required_evidence_present:current.length===rule.required_current_evidence.length,no_missing_required_evidence:rule.allow_missing_required_evidence||missing.length===0,no_contradicting_evidence:rule.allow_contradicting_case_evidence||contradicting.length===0},sufficient=Object.values(checks).every(Boolean),missingRefs=[!checks.same_case?"WRONG_CASE_CONTEXT":null,!checks.crop_matches?"WRONG_CROP_CONTEXT":null,!checks.candidate_matches?"CANDIDATE_NOT_APPLICABLE":null,!checks.candidate_state_supported?"STEP_C_CANDIDATE_NOT_SUPPORTED":null,!checks.human_review_matches?"HUMAN_REVIEW_REQUIRED":null,!checks.supporting_case_evidence_present?"MISSING_CASE_EVIDENCE":null,!checks.current_required_evidence_present?"MISSING_CURRENT_WATER_CONTEXT":null,!checks.no_missing_required_evidence?"STEP_C_REQUIRED_EVIDENCE_MISSING":null].filter(Boolean);
      const manifest=this.getManifest(),adjudication={purpose:rule.purpose,state:sufficient?rule.sufficient_state:"INSUFFICIENT_FOR_NARROWING",rule_id:rule.rule_id,rule_version:this.version,provider_id:manifest.provider_id,provider_version:manifest.provider_version,provider_content_hash:manifest.content_hash,authority:manifest.authority,review_state:manifest.review_state,source_refs:rule.source_refs,source_bindings:this.package.sources,current_authoritative_evidence_refs:[...new Set([...supporting,...current])],candidate_context:candidate?{candidate_id:candidate.candidate_id,concept_id:candidate.concept_id,state:candidate.state}:null,finding_review_ref:review?.review_id??null,checks,missing_evidence_refs:missingRefs,contradicting_evidence_refs:contradicting,limitations:rule.limitations,no_llm_authority:true,no_management_provider_authority:true};
      if(sufficient)return adjudication;
      if(rule===this.package.rules.at(-1))return adjudication;
    }
    return null;
  }
}
...
export class InvestigationIntelligenceEngine {
  constructor({ruleProvider=new GovernedInvestigationRuleProvider(),candidateProvider=loadCandidateProvider(),clock=()=>new Date()}={}){this.ruleProvider=ruleProvider;this.candidateProvider=candidateProvider;this.clock=clock;}
  assess(bundle){
    ...
    const config=normalizeProvider(this.ruleProvider,bundle),providerResult=this.candidateProvider?.nominate?.(bundle)??{provider_manifest:null,nominated_candidates:[],comparison_rules:[],adjudication_rules:[],knowledge_gaps:[],conflicts:[]},sources=sourceIndex(bundle),identityMap=new Map(),suppliedCandidates=[];
    for(const candidate of bundle.candidates??[]){const identity=candidate.concept_id??`AUTHORED:${candidate.candidate_id}`,copy={...candidate,concept_id:candidate.concept_id??null,nomination_sources:[{type:candidate.authorship??"EXPLICITLY_AUTHORED",candidate_id:candidate.candidate_id,concept_id:candidate.concept_id??null,review_state:candidate.review_state}]};identityMap.set(identity,copy);suppliedCandidates.push({candidate_id:candidate.candidate_id,concept_id:candidate.concept_id??null,authorship:candidate.authorship??"EXPLICITLY_AUTHORED"});}
    for(const nomination of providerResult.nominated_candidates??[]){const existing=identityMap.get(nomination.concept_id);if(existing){existing.nomination_sources.push(...nomination.nomination_sources);continue;}identityMap.set(nomination.concept_id,{candidate_id:nomination.candidate_id,concept_id:nomination.concept_id,candidate_class:nomination.candidate_class,label:nomination.label,support_state:"OPEN",review_state:"UNREVIEWED",evidence_links:[],authorship:"GOVERNED_PROVIDER_NOMINATION",nomination_sources:structuredClone(nomination.nomination_sources)});}
    const merged=[...identityMap.values()],candidates=new Map(merged.map((candidate)=>[candidate.candidate_id,candidate])),candidateIdByConcept=new Map(merged.filter((candidate)=>candidate.concept_id).map((candidate)=>[candidate.concept_id,candidate.candidate_id]));
    const provenanceFor=(rule)=>({rule_id:rule.rule_id,rule_hash:rule.rule_hash,provider_id:providerResult.provider_manifest?.provider_id??null,provider_version:providerResult.provider_manifest?.provider_version??null,provider_content_hash:providerResult.provider_manifest?.content_hash??null,source_refs:rule.knowledge_source_refs??rule.source_refs,source_concept_claim_ids:rule.source_concept_claim_ids,authority:rule.authority,review_state:rule.review_state,scope:rule.scope,materialized_at:rule.materialized_at});
    const providerRules=(providerResult.adjudication_rules??[]).map((rule)=>({rule_id:rule.rule_id,candidate_id:candidateIdByConcept.get(rule.candidate_concept_id),dimension:rule.dimension,relation:rule.relation,constraint:rule.constraint,reason:rule.reason,source_refs:rule.source_refs,semantic_basis:rule.semantic_basis,negative_feature:rule.negative_feature,inspection_method_appropriate:rule.inspection_method_appropriate,provider_provenance:provenanceFor(rule)})).filter((rule)=>rule.candidate_id);
    const comparisonRules=(providerResult.comparison_rules??[]).flatMap((rule)=>{const related=rule.candidate_concept_ids.map((conceptId)=>candidateIdByConcept.get(conceptId)).filter(Boolean);if(!related.length)return[];const candidateId=related[0],fingerprint=hash({rule_id:rule.rule_id,related}).slice(0,20),gapRef=`provider-gap-${fingerprint}`;return [{rule_id:`provider-comparison-${fingerprint}`,candidate_id:candidateId,dimension:rule.dimension,relation:"REQUIRED_BUT_MISSING",constraint:rule.relation,reason:`Governed comparison rule ${rule.rule_id} identifies distinguishing evidence.`,source_refs:[],missing_evidence:{gap_ref:gapRef,concept:rule.evidence_concept,importance:rule.relation==="REQUIRED"?"REQUIRED":"USEFUL",reason:rule.next_best_evidence.reason},next_best_evidence:{...rule.next_best_evidence,related_candidate_ids:related,existing_evidence_gap_refs:[gapRef]},provider_provenance:provenanceFor(rule)}];});
    const rules=[...config.rules,...providerRules,...comparisonRules].map((rule)=>validatedRule(rule,candidates,sources)),uncertainty=[...(providerResult.conflicts??[])],requests=[];
    const assessments=[...candidates.values()].sort((a,b)=>a.candidate_id.localeCompare(b.candidate_id)).map((candidate)=>{
      ...
      for(const rule of rules.filter((item)=>item.candidate_id===candidate.candidate_id)){
        ...
        for(const sourceRef of rule.source_refs){const entry=relationEntry({relation,dimension:rule.dimension,constraint:rule.constraint,reason:rule.reason,ruleRef:rule.rule_id,sourceRef,ruleProvenance:rule.provider_provenance});addRelation(relations,entry);}
      ...
      const providerNominated=candidate.nomination_sources.some((item)=>item.type==="GOVERNED_PROVIDER");
      return {candidate_id:candidate.candidate_id,concept_id:candidate.concept_id??null,candidate_class:candidate.candidate_class,label:candidate.label??null,state:candidateState(candidate,supporting,contradicting,missingEvidence,config),persisted_hypothesis_state:candidate.support_state,review_state:candidate.review_state,consideration_basis:{type:providerNominated?candidate.nomination_sources.length>1?"MERGED_AUTHORED_AND_GOVERNED_CANDIDATE":"GOVERNED_PROVIDER_NOMINATION":"PERSISTED_AUTHORED_CANDIDATE",candidate_id:candidate.candidate_id,concept_id:candidate.concept_id??null},nomination_provenance:structuredClone(candidate.nomination_sources),compatibility,supporting_evidence:supporting,contradicting_evidence:contradicting,neutral_evidence:neutral,missing_evidence:missingEvidence,resolution_role:null,resolution_level:null};
    });
    ...
    const providerManifest=providerResult.provider_manifest??null,combinedRuleVersion=providerManifest?`${config.rule_version}+${providerManifest.provider_id}@${providerManifest.provider_version}`:config.rule_version,combinedRuleHash=hash({rule_authority:config.rule_authority,rule_review_state:config.rule_review_state,rules:config.rules,stop_condition:config.stop_condition,sufficiency_state:config.sufficiency_state,field_evidence_exhausted:config.field_evidence_exhausted,specialist_confirmation_required:config.specialist_confirmation_required,next_best_evidence:config.next_best_evidence,provider_content_hash:providerManifest?.content_hash??null,provider_adjudication_rules:providerResult.adjudication_rules??[],provider_comparison_rules:providerResult.comparison_rules??[]});
    return {authority:"SERVER_DERIVED_INVESTIGATION_ASSESSMENT",scientific_input:"AUTHORITATIVE_INVESTIGATION_BUNDLE_ONLY",scope:{user_id:bundle.user_id,field_id:bundle.field_id,season_id:bundle.season_id,case_id:bundle.cases?.length===1?bundle.cases[0].case_id:null,observation_ids:(bundle.observations??[]).map((item)=>item.observation_id)},field_context_reused:{crop:bundle.field_context?.crop??null,stage_assessment_id:bundle.stage_assessment?.stage_assessment_id??null,evidence_refs:(bundle.evidence??[]).map((item)=>item.evidence_id),management_event_refs:(bundle.management_events??[]).map((item)=>item.management_event_id),new_user_input_required_for_nomination:false},result_state:resultState,candidate_ordering:"STABLE_IDENTIFIER_ONLY_NO_RANK",nominated_candidates:assessments.filter((item)=>item.nomination_provenance.some((source)=>source.type==="GOVERNED_PROVIDER")).map((item)=>({candidate_id:item.candidate_id,concept_id:item.concept_id,candidate_class:item.candidate_class,nomination_provenance:item.nomination_provenance.filter((source)=>source.type==="GOVERNED_PROVIDER")})),supplied_candidates:suppliedCandidates,candidate_assessments:assessments,provider_manifest:providerManifest,comparison_rules:(providerResult.comparison_rules??[]).map((rule)=>({...structuredClone(rule),provider_id:providerManifest?.provider_id??null,provider_version:providerManifest?.provider_version??null,provider_content_hash:providerManifest?.content_hash??null})),knowledge_gaps:providerResult.knowledge_gaps??[],evidence_sufficiency:{state:sufficiency,purpose_states:{NARROWING:narrowingReady?"SUFFICIENT_FOR_NARROWING":"INSUFFICIENT_FOR_NARROWING",MANAGEMENT_OPTION_REVIEW:sufficiency==="SUFFICIENT_FOR_MANAGEMENT_OPTION_REVIEW"?sufficiency:"INSUFFICIENT_FOR_NARROWING",HUMAN_FINDING:sufficiency==="SUFFICIENT_FOR_HUMAN_FINDING"?sufficiency:config.specialist_confirmation_required?"REQUIRES_SPECIALIST_CONFIRMATION":"INSUFFICIENT_FOR_NARROWING"},reason:sufficiencyReason,limiting_gaps:[...new Map(assessments.flatMap((item)=>item.missing_evidence).map((gap)=>[gap.gap_ref,gap])).values()]},next_best_evidence:next,stop_condition:stop,uncertainty,generated_at:this.clock().toISOString(),source_bundle_hash:hash(authoritativeProjection(bundle)),rule_version:combinedRuleVersion,rule_authority:config.rule_authority,rule_review_state:config.rule_review_state,rule_hash:combinedRuleHash,boundaries:{candidate_is_diagnosis:false,nomination_is_support:false,ranking_is_probability:false,support_is_proof:false,contradiction_is_impossibility:false,missing_is_negative_evidence:false,temporal_association_is_causation:false,management_history_is_attribution:false,outcome_is_efficacy:false,failure_is_resistance:false,management_option_selected:false,openai_candidate_generation:false,guidance_generated:false,chemical_output_generated:false}};
  }
}
...
export class InvestigationIntelligenceService {
  constructor(db,bundleProvider,{ruleProvider=new GovernedInvestigationRuleProvider(),candidateProvider=loadCandidateProvider(),clock=()=>new Date(),idProvider=()=>randomUUID()}={}){this.db=db;this.bundleProvider=bundleProvider;this.clock=clock;this.idProvider=idProvider;this.ruleProvider=ruleProvider;this.engine=new InvestigationIntelligenceEngine({ruleProvider,candidateProvider,clock});}
  ...
  withPurposeSufficiency(assessment,bundle,reviewHistory){const adjudication=this.ruleProvider.adjudicateManagementSufficiency?.(bundle,assessment,reviewHistory)??null;if(!adjudication)return{...assessment,review_history:reviewHistory};const sufficient=adjudication.state==="SUFFICIENT_FOR_MANAGEMENT_OPTION_REVIEW",prior=assessment.evidence_sufficiency;return{...assessment,evidence_sufficiency:{...prior,state:sufficient?adjudication.state:prior.state,purpose_states:{...prior.purpose_states,MANAGEMENT_OPTION_REVIEW:adjudication.state},reason:sufficient?"The governed Step C provider declares the bounded Case sufficient for Management Option Review; no management option or action is selected.":prior.reason,limiting_gaps:sufficient?prior.limiting_gaps:[...prior.limiting_gaps,...adjudication.missing_evidence_refs.filter((gap_ref)=>!prior.limiting_gaps.some((item)=>item.gap_ref===gap_ref)).map((gap_ref)=>({gap_ref,concept:gap_ref,importance:"REQUIRED",reason:"The governed Step C management-sufficiency rule is not satisfied.",rule_ref:adjudication.rule_id}))]},management_sufficiency_adjudication:adjudication,review_history:reviewHistory};}
  assess(userId,scope){
    ...
    if(latest&&!latest.stale_at&&latest.source_bundle_hash===derived.source_bundle_hash&&latest.rule_version===derived.rule_version&&latest.rule_hash===derived.rule_hash){const assessment=JSON.parse(latest.assessment_json),reviews=this.reviews(userId,assessment.assessment_id);return this.withPurposeSufficiency({...assessment,stale:false},bundle,reviews);}
    ...
    return this.withPurposeSufficiency(assessment,bundle,[]);
  }
  history(userId,scope){
    id(userId,"user_id");const fieldId=id(scope.field_id,"field_id"),seasonId=id(scope.season_id,"season_id"),caseId=id(scope.case_id,"case_id"),bundle=this.bundleProvider.getBundle(userId,{field_id:fieldId,season_id:seasonId,case_id:caseId});
    return this.db.prepare("SELECT assessment_json,stale_at FROM investigation_assessments WHERE owner_user_id=? AND field_id=? AND season_id=? AND case_id=? ORDER BY assessment_revision").all(userId,fieldId,seasonId,caseId).map((row)=>{const assessment=JSON.parse(row.assessment_json),reviews=this.reviews(userId,assessment.assessment_id),result={...assessment,stale:Boolean(row.stale_at),stale_at:row.stale_at};return row.stale_at?{...result,review_history:reviews}:this.withPurposeSufficiency(result,bundle,reviews);});
  }
  review(userId,input){
