...
export async function startServer({ port, host, dbPath, exportDir, uploadDir: configuredUploadDir } = {}) {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/api/pilot/investigation-records") { const payload = await readJson(request, 256_000); if (!payload || Object.keys(payload).some((key) => !["record_type","record"].includes(key)) || Object.keys(payload).length !== 2) throw new Error("invalid investigation record request"); const record = store.createInvestigationRecord(session.user_id,payload.record_type,payload.record); return json(response,201,{ status:"CREATED", authority:"SERVER", record_type:payload.record_type, record }); }
