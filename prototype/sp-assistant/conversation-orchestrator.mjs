...
import { composeNaturalFieldGuidance, naturalVisualReceipt } from "./conversation-presentation.mjs";
...
export const CONVERSATION_ENUMS = Object.freeze({
  ...
  intents: ["FIELD_OBSERVATION", "FIELD_STATUS_UPDATE", "QUESTION", "VISUAL_CHECK", "GUIDANCE_RESPONSE", "CASE_FOLLOW_UP", "FOLLOW_UP_SCHEDULING", "REMINDER_ACTION", "CONTEXT_SELECTION", "USER_HYPOTHESIS", "MANAGEMENT_QUERY", "MANAGEMENT_SELECTION", "ACTION_CONFIRMATION", "OUTCOME_REPORT", "KNOWLEDGE_QUERY", "NAVIGATION_REQUEST", "SOCIAL_OR_OTHER", "CANNOT_ANSWER", "USER_CORRECTION"],
  extractionStatuses: ["EXPLICIT", "AMBIGUOUS", "UNSUPPORTED_INFERENCE"],
  responseTypes: ["ACKNOWLEDGEMENT", "CLARIFICATION", "FIELD_GUIDANCE", "VISUAL_GUIDANCE", "INVESTIGATION_SUMMARY", "UNCERTAINTY_SUMMARY", "EXPERT_HANDOFF", "LAB_HANDOFF", "NO_ADDITIONAL_ACTION", "MANAGEMENT_REVIEW_SUMMARY", "DECISION_CONFIRMED", "ACTION_RECORDED", "OUTCOME_RECORDED", "FOLLOW_UP_SCHEDULED", "REMINDER_UPDATED", "GENERAL_KNOWLEDGE", "SYSTEM_LIMITATION", "ERROR_SAFE"],
  uiActions: ["NONE", "OPEN_FIELD", "OPEN_CASE", "OPEN_CAMERA", "OPEN_GUIDANCE", "OPEN_VISUAL_REVIEW", "OPEN_KNOWLEDGE"],
  governedActions: ["ACK", "RECORD_EXPLICIT_FACTS", "ASK_ONE_CLARIFICATION", "PRESENT_CURRENT_GUIDANCE", "REQUEST_FIELD_CHECK", "REQUEST_VISUAL_EVIDENCE", "CHECK_VISUAL_EVIDENCE", "PRESENT_INVESTIGATION_SUMMARY", "PRESENT_UNCERTAINTY_SUMMARY", "REQUEST_HUMAN_REVIEW", "REQUEST_EXPERT_REVIEW", "REQUEST_LAB_EVIDENCE", "NO_ADDITIONAL_ACTION", "NAVIGATE", "KNOWLEDGE_LOOKUP", "PRESENT_MANAGEMENT_REVIEW", "CONFIRM_MANAGEMENT_DECISION", "RECORD_MANAGEMENT_ACTION", "RECORD_OUTCOME", "SCHEDULE_FOLLOW_UP", "UPDATE_REMINDER"],
...
export class ConversationProvider {
  ...
  async compose() { throw new ConversationProviderError("CONVERSATION_UNAVAILABLE", "conversation composer is not configured"); }
}

export function createTestOnlyConversationProvider(proposal,composer=null) {
  return new class extends ConversationProvider {
    ...
    async compose(input) { if(!composer)throw new ConversationProviderError("CONVERSATION_UNAVAILABLE","test composer not configured");return structuredClone(typeof composer==="function"?await composer(structuredClone(input)):composer); }
  }();
}
...
export class OpenAIConversationProvider extends ConversationProvider {
  ...
  async compose({ responsePlan }) {
    if (!this.available) throw new ConversationProviderError("CONVERSATION_UNAVAILABLE", "OpenAI conversation provider is not configured");
    const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),this.timeoutMs);
    const transitions=["", "โอเคครับ", "รับทราบครับ", "ขอบคุณที่บอกครับ", "ค่อย ๆ ตรวจไปทีละอย่างนะครับ"];
    const schema={type:"object",additionalProperties:false,required:["transition"],properties:{transition:{type:"string",enum:transitions}}};
    const body={model:this.model,store:false,temperature:0,instructions:"Compose a natural Thai transition for the governed response. Choose exactly one permitted transition. The server owns every fact, question, action, diagnosis and recommendation. Do not add text or alter the required message.",input:[{role:"user",content:[{type:"input_text",text:JSON.stringify(responsePlan)}]}],text:{format:{type:"json_schema",name:"governed_response_transition",strict:true,schema}}};
    try{const response=await this.fetcher("https://api.openai.com/v1/responses",{method:"POST",headers:{authorization:`Bearer ${this.apiKey}`,"content-type":"application/json"},body:JSON.stringify(body),signal:controller.signal});if(!response.ok)throw new ConversationProviderError("CONVERSATION_UNAVAILABLE","conversation composer is unavailable");const payload=await response.json(),text=outputText(payload);if(!text)throw new ConversationProviderError("MALFORMED_PROVIDER_OUTPUT","composer returned no output");let parsed;try{parsed=JSON.parse(text);}catch{throw new ConversationProviderError("MALFORMED_PROVIDER_OUTPUT","composer returned invalid JSON");}return parsed;}catch(error){if(error instanceof ConversationProviderError)throw error;throw new ConversationProviderError(controller.signal.aborted?"CONVERSATION_TIMEOUT":"CONVERSATION_UNAVAILABLE","composer request failed");}finally{clearTimeout(timeout);}
  }
}
...
export function initializeConversationSchema(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS governed_context_conflicts (
      context_conflict_id TEXT PRIMARY KEY, owner_user_id TEXT NOT NULL, field_id TEXT NOT NULL,
      season_id TEXT NOT NULL, case_id TEXT, turn_id TEXT NOT NULL UNIQUE,
      dimension TEXT NOT NULL, stored_code TEXT NOT NULL, reported_code TEXT NOT NULL,
      reported_text TEXT NOT NULL, status TEXT NOT NULL, created_at TEXT NOT NULL
    );
  `);
...
function validateInput(raw) {
  ...
  allowed(raw, ["request_id","conversation_id","turn_id","message","entry_point","field_id","season_id","case_id","observation_id","guidance_id","image_evidence_id","requested_visual_target","route_context","explicit_reference","audience","expected_conversation_revision","corrects_turn_id","management_option_id","decision_id","management_action_id","performed_at","outcome_phase","follow_up_plan_id","reminder_id","due_at","timezone","remain_general"], "conversation request");
...
function localInterpretation(message, input) {
  ...
  let intent = /(?:แก้|ขอแก้|แก้ไข|เมื่อกี้.*(?:ไม่ใช่|ผิด)|จริงๆแล้ว|ที่ถูกคือ)/iu.test(message) ? "USER_CORRECTION" : /^(?:ไม่รู้|ไม่ได้ดู|จำไม่ได้|ไม่มีรูป|ไว้ดูทีหลัง)[.!\s]*$/iu.test(message) ? "CANNOT_ANSWER" : /รับทราบเตือน|ปิดเตือน|ทำรายการติดตามนี้แล้ว|ยกเลิกเตือน|เลิกเตือน/iu.test(message) ? "REMINDER_ACTION" : /เตือน(?:ฉัน|ผม|เรา)?|กลับมาดูอีก|นัดติดตาม/iu.test(message) ? "FOLLOW_UP_SCHEDULING" : /ทำแล้ว|ดำเนินการแล้ว|ลงมือแล้ว/iu.test(message) ? "ACTION_CONFIRMATION" : /ดีขึ้น|น้อยลง|ลดลง|มากขึ้น|เพิ่มขึ้น|แย่ลง|เท่าเดิม|ไม่เปลี่ยน/iu.test(message) ? "OUTCOME_REPORT" : /(?:เลือก|เอา)(?:ทาง|ตัวเลือก)|ติดตามต่อ|ขอผู้เชี่ยวชาญ|ยังไม่ทำตอนนี้/iu.test(message) ? "MANAGEMENT_SELECTION" : /ยา|สารเคมี|พ่น|กำจัด|ใช้สาร|โดส|อัตรา|ทางเลือก|จัดการ|ทำอะไรได้บ้าง/iu.test(message) ? "MANAGEMENT_QUERY" : input.image_evidence_id || /ถ่ายแบบนี้|ดูรูป|ดูภาพ|รูปนี้|ภาพนี้/iu.test(message) ? "VISUAL_CHECK" : facts.length ? "FIELD_OBSERVATION" : userHypothesis ? "USER_HYPOTHESIS" : /ความรู้|คืออะไร|ทั่วไป/iu.test(message) ? "KNOWLEDGE_QUERY" : /เปิด|ไปหน้า/iu.test(message) ? "NAVIGATION_REQUEST" : /ขอบคุณ|สวัสดี/iu.test(message) ? "SOCIAL_OR_OTHER" : "QUESTION";
  return { interpretation_version:"server-governed-thai-interpretation/v1", intent, server_explicit_f2_intent:["MANAGEMENT_SELECTION","ACTION_CONFIRMATION","OUTCOME_REPORT"].includes(intent), server_explicit_g_intent:["FOLLOW_UP_SCHEDULING","REMINDER_ACTION"].includes(intent), explicit_facts:facts, user_hypothesis:userHypothesis, unsupported_inferences:[], raw_message_is_evidence:false };
}

function stageClaim(message){const mappings=[["BOOTING",/ตั้งท้อง/iu],["TILLERING",/แตกกอ/iu],["HEADING",/ออกรวง/iu]];for(const [code,pattern] of mappings){const match=pattern.exec(message);if(match)return{code,text:match[0]};}return null;}
...
const SAFE_TRANSITIONS=new Set(["", "โอเคครับ", "รับทราบครับ", "ขอบคุณที่บอกครับ", "ค่อย ๆ ตรวจไปทีละอย่างนะครับ"]);
export function guardComposedResponse(proposal,plan){
  object(proposal,"composed response");allowed(proposal,["transition"],"composed response");
  if(!SAFE_TRANSITIONS.has(proposal.transition))throw new ConversationProviderError("PROVIDER_POLICY_REJECTION","composer attempted unsupported wording");
  const locked=boundedText(plan.required_text,"required_text"),transition=proposal.transition;
  const text=transition&&!locked.startsWith(transition)?`${transition} ${locked}`:locked;
  if(plan.required_question&& !text.includes(plan.required_question))throw new ConversationProviderError("PROVIDER_POLICY_REJECTION","composer omitted governed question");
  return {text,question:plan.required_question??null,composition_state:"GUARDED_PROVIDER_COMPOSITION",transition};
}
function currentEvidenceAction(primary,pkg){
  if(!["REQUEST_VISUAL_EVIDENCE","REQUEST_FIELD_CHECK"].includes(primary.action))return null;
  const visual=pkg.visual?.active_request;
  if(primary.action==="REQUEST_VISUAL_EVIDENCE"&&visual?.request_type==="PHOTO")return{target:visual.target,method:"IMAGE",goal:visual.why_needed,where:visual.where,how:visual.how,quality_requirement:visual.completion_condition,related_guidance_id:visual.related_guidance_id,one_primary_action:true,fallback:"USER_CAN_DEFER_OR_REPORT_UNAVAILABLE"};
  const guidance=pkg.guidance;if(!guidance)return null;
  return{target:guidance.what_to_inspect??guidance.title,method:"FIELD_CHECK",goal:guidance.why_now,where:guidance.where_to_inspect,how:guidance.how_to_inspect,quality_requirement:null,related_guidance_id:guidance.guidance_id,one_primary_action:true,fallback:"USER_CAN_DEFER_OR_REPORT_UNAVAILABLE"};
}
...
export class GovernedConversationOrchestrator {
  constructor(db, { backbone, captureAdapter = null, assessmentService, guidanceService, visualEvidenceService, visualPerceptionService, managementService, decisionActionOutcomeService = null, followupReminderService = null, provider = new ConversationProvider(), fieldCaptureAlpha = false, clock = () => new Date(), idProvider = () => randomUUID() } = {}) {
    this.db=db; this.backbone=backbone; this.captureAdapter=captureAdapter??new GovernedConversationCaptureAdapter(backbone); this.assessmentService=assessmentService; this.guidanceService=guidanceService; this.visualEvidenceService=visualEvidenceService; this.visualPerceptionService=visualPerceptionService; this.managementService=managementService; this.decisionActionOutcomeService=decisionActionOutcomeService; this.followupReminderService=followupReminderService; this.provider=provider; this.fieldCaptureAlpha=fieldCaptureAlpha===true; this.clock=clock; this.idProvider=idProvider; this.inFlight=new Map();
  }
  now() { return this.clock().toISOString(); }
  recordContextConflict(userId,context,turnId,message){const claim=stageClaim(message);if(!claim)return null;const stage=this.db.prepare("SELECT crop_stage_json FROM stage_assessments WHERE owner_user_id=? AND field_id=? AND season_id=?").get(userId,context.field_id,context.season_id),stored=stage?.crop_stage_json?JSON.parse(stage.crop_stage_json)?.code:null;if(!stored||stored===claim.code)return null;const event={context_conflict_id:`context-conflict-${this.idProvider()}`,owner_user_id:userId,field_id:context.field_id,season_id:context.season_id,case_id:context.case_id??null,turn_id:turnId,dimension:"CROP_STAGE",stored_code:stored,reported_code:claim.code,reported_text:claim.text,status:"UNRESOLVED",created_at:this.now()};this.db.prepare("INSERT OR IGNORE INTO governed_context_conflicts VALUES(?,?,?,?,?,?,?,?,?,?,?,?)").run(...Object.values(event));return event;}
  ...
  ensureConversation(userId,rawInput={}) { identifier(userId,"user_id");object(rawInput,"conversation context");allowed(rawInput,["field_id","season_id","case_id","entry_point"],"conversation context");const fieldId=identifier(rawInput.field_id,"field_id"),seasonId=identifier(rawInput.season_id,"season_id"),caseId=identifier(rawInput.case_id,"case_id"),entryPoint=member(rawInput.entry_point??"CASE",new Set(["CASE","IMAGE"]),"entry_point");this.backbone.getBundle(userId,{field_id:fieldId,season_id:seasonId,case_id:caseId});const existing=this.db.prepare("SELECT * FROM governed_conversations WHERE owner_user_id=? AND field_id=? AND season_id=? AND case_id=? AND status='ACTIVE' ORDER BY updated_at DESC LIMIT 1").get(userId,fieldId,seasonId,caseId);if(existing)return safeConversation(existing);const now=this.now(),conversationId=`conversation-${this.idProvider()}`,contextHash=hash({field_id:fieldId,season_id:seasonId,case_id:caseId});this.db.prepare("INSERT INTO governed_conversations VALUES(?,?,?,?,?,?,?,?,?,?,?)").run(conversationId,userId,fieldId,seasonId,caseId,entryPoint,"ACTIVE",contextHash,1,now,now);return safeConversation(this.conversation(userId,conversationId)); }
  ...
  supersedeCorrectedFacts(userId,input,turnId) {
    if(!input.corrects_turn_id)return [];
    const prior=this.db.prepare("SELECT response_json FROM governed_conversation_turns WHERE turn_id=? AND owner_user_id=?").get(input.corrects_turn_id,userId),refs=prior?JSON.parse(prior.response_json).fact_record_references??[]:[],superseded=[];
    for(const ref of refs){const changed=this.db.prepare("UPDATE investigation_evidence SET review_state='SUPERSEDED',updated_at=?,revision=revision+1 WHERE evidence_id=? AND owner_user_id=? AND review_state<>'SUPERSEDED'").run(this.now(),ref.record_id,userId).changes;if(changed)superseded.push({...ref,superseded_by_turn_id:turnId});}
    return superseded;
  }
  contextPackage(userId,context,input) {
    ...
    let assessment=null,guidance=null,management=null,decisionActionOutcome=null,followupReminder=null;if(context.case_id){const governedScope={field_id:context.field_id,season_id:context.season_id,case_id:context.case_id};assessment=this.assessmentService.assess(userId,governedScope);guidance=this.guidanceService.current(userId,governedScope);management=this.managementService.current(userId,governedScope);decisionActionOutcome=this.decisionActionOutcomeService?.context(userId,governedScope)??null;followupReminder=this.followupReminderService?.context(userId,governedScope)??null;}
  ...
  gResponse(userId,input,interpretation,pkg) {
    if(!this.followupReminderService||!pkg.identity.case_id)return null;
    const scope={field_id:pkg.identity.field_id,season_id:pkg.identity.season_id,case_id:pkg.identity.case_id};
    if(["FOLLOW_UP_SCHEDULING","REMINDER_ACTION"].includes(interpretation.intent)&&!interpretation.server_explicit_g_intent)return{action:"ASK_ONE_CLARIFICATION",response_type:"CLARIFICATION",text:"กรุณายืนยันเวลาหรือการจัดการรายการเตือนด้วยถ้อยคำที่ชัดเจนครับ",question:"ต้องการตั้งเวลาหรือจัดการรายการเตือนใดครับ",ui_action:"OPEN_CASE",record:null};
    if(interpretation.intent==="FOLLOW_UP_SCHEDULING"){
      const relative=/อีก\s*(\d{1,3})\s*วัน/iu.exec(input.message),days=relative?Number(relative[1]):null,dueAt=input.due_at??(days!=null?new Date(this.clock().getTime()+days*86_400_000).toISOString():null);if(!dueAt)return{action:"ASK_ONE_CLARIFICATION",response_type:"CLARIFICATION",text:"ต้องการให้เตือนวันและเวลาใดครับ ระบบจะไม่เดาเวลาให้",question:"ต้องการให้เตือนวันและเวลาใดครับ",ui_action:"OPEN_CASE",record:null};
      const plans=pkg.follow_up_reminder?.current_follow_up_plans??[],base=input.follow_up_plan_id?plans.find((item)=>item.follow_up_plan_id===input.follow_up_plan_id):plans.length===1?plans[0]:null;if(plans.length>1&&!base)return{action:"ASK_ONE_CLARIFICATION",response_type:"CLARIFICATION",text:"มีหลายแผนติดตาม ต้องการตั้งเตือนสำหรับแผนใดครับ",question:"ต้องการตั้งเตือนสำหรับแผนใดครับ",ui_action:"OPEN_CASE",record:null};const guidance=pkg.guidance;if(!base&&!guidance)return{action:"ASK_ONE_CLARIFICATION",response_type:"CLARIFICATION",text:"ยังไม่พบแผนหรือคำแนะนำการตรวจที่ระบุว่าจะกลับไปดูอะไร กรุณาเลือกแผนติดตามหนึ่งรายการครับ",question:"ต้องการติดตามเรื่องใดครับ",ui_action:"OPEN_CASE",record:null};
      const sourceRef=`conversation-turn:${input.request_id}`,what=base?.what_to_check??[guidance.what_to_inspect??guidance.title],where=base?.where_to_check??guidance.where_to_inspect??"พื้นที่เคสที่ผู้ใช้เลือก",result=this.followupReminderService.createPlan(userId,{request_id:`${input.request_id}-g-plan`,...scope,source_type:base?.source_type??"HUMAN_AUTHORED_FOLLOW_UP",source_ref:base?.source_ref??sourceRef,guidance_id:base?.guidance_id??guidance?.guidance_id,purpose:base?.purpose??`ติดตามตามคำขอของผู้ใช้: ${input.message}`,target_subject:base?.target_subject??guidance?.title??"รายการติดตามที่ผู้ใช้ระบุ",what_to_check:what,where_to_check:where,plant_part:base?.plant_part,measurement_concept:base?.measurement_concept,unit:base?.unit,denominator:base?.denominator,sampling_method:base?.sampling_method,sampling_context:base?.sampling_context??{},comparison_target:base?.comparison_target,visual_target:base?.visual_target,expected_evidence_type:base?.expected_evidence_type??"OBSERVATION",timing_mode:"EXACT_TIME",timing_basis:days!=null?`USER_EXPLICIT_RELATIVE_DAYS:${days}`:"USER_EXPLICIT_DUE_AT",due_at:dueAt,timing_authority:"HUMAN_AUTHORED",timing_source_ref:sourceRef,limitations:[...(base?.limitations??[]),"เวลานี้มาจากผู้ใช้ ไม่ใช่ข้อสรุปว่าเป็นช่วงเวลาที่เหมาะสมทางวิทยาศาสตร์"],status:"ACTIVE",supersedes_follow_up_plan_id:base?.follow_up_plan_id??null,create_reminder:true,timezone:input.timezone??"Asia/Bangkok",reminder_reason:base?.purpose??guidance?.title??"ติดตามตามคำขอผู้ใช้"});return{action:"SCHEDULE_FOLLOW_UP",response_type:"FOLLOW_UP_SCHEDULED",text:"บันทึกแผนและตั้งการเตือนตามเวลาที่คุณระบุแล้วครับ เวลานี้เป็นเวลาที่ผู้ใช้กำหนด ไม่ใช่ช่วงเวลาที่ระบบสรุปว่าเหมาะสมทางวิทยาศาสตร์",question:null,ui_action:"OPEN_CASE",record:{follow_up_plan_id:result.plan.follow_up_plan_id,reminder_id:result.reminder?.reminder_id??null}};
    }
    if(interpretation.intent==="REMINDER_ACTION"){
      const reminders=pkg.follow_up_reminder?.current_reminders??[],current=input.reminder_id?reminders.find((item)=>item.reminder_id===input.reminder_id):reminders.length===1?reminders[0]:null;if(!current)return{action:"ASK_ONE_CLARIFICATION",response_type:"CLARIFICATION",text:"ต้องการจัดการรายการเตือนใดครับ",question:"ต้องการจัดการรายการเตือนใดครับ",ui_action:"OPEN_CASE",record:null};const action=/รับทราบ/iu.test(input.message)?"ACKNOWLEDGE":/ทำรายการติดตามนี้แล้ว/iu.test(input.message)?"COMPLETE":/ยกเลิก/iu.test(input.message)?"CANCEL":"DISMISS",reminder=this.followupReminderService.actOnReminder(userId,{request_id:`${input.request_id}-g-reminder`,reminder_id:current.reminder_id,action,expected_revision:current.revision});return{action:"UPDATE_REMINDER",response_type:"REMINDER_UPDATED",text:action==="COMPLETE"?"บันทึกว่ารายการเตือนเสร็จแล้วครับ สถานะนี้ไม่สร้างหลักฐาน ผลลัพธ์ หรือปิดเคสโดยอัตโนมัติ":"บันทึกสถานะรายการเตือนแล้วครับ สถานะนี้ไม่เปลี่ยนข้อสรุปทางวิทยาศาสตร์หรือสถานะเคส",question:null,ui_action:"OPEN_CASE",record:{reminder_id:reminder.reminder_id,follow_up_plan_id:reminder.follow_up_plan_id}};
    }
    return null;
  }
  f2Response(userId,input,interpretation,pkg) {
    if(!this.decisionActionOutcomeService||!pkg.management)return null;
    const scope={field_id:pkg.identity.field_id,season_id:pkg.identity.season_id,case_id:pkg.identity.case_id},memory=pkg.decision_action_outcome;
    if(["MANAGEMENT_SELECTION","ACTION_CONFIRMATION","OUTCOME_REPORT"].includes(interpretation.intent)&&!interpretation.server_explicit_f2_intent)return {action:"ASK_ONE_CLARIFICATION",response_type:"CLARIFICATION",text:"กรุณายืนยันการเลือก การดำเนินการ หรือผลที่เห็นด้วยถ้อยคำที่ชัดเจนครับ",question:"ต้องการยืนยันสิ่งใดครับ",ui_action:"OPEN_CASE",record:null};
    if(interpretation.intent==="MANAGEMENT_SELECTION"){
      const mappings=[[/ติดตามต่อ/iu,"CONTINUE_MONITORING"],[/ผู้เชี่ยวชาญ/iu,"EXPERT_REVIEW"],[/ยังไม่ทำ|ไม่ดำเนินการ/iu,"NO_ACTION_CURRENTLY_JUSTIFIED"],[/เขตกรรม|น้ำ|วัฒนธรรม/iu,"CULTURAL_MANAGEMENT"],[/เชิงกล|ถอน|ตัด/iu,"MECHANICAL_MANAGEMENT"],[/ชีวภาพ/iu,"BIOLOGICAL_MANAGEMENT"],[/สารเคมี|ทางสาร|ทางยา|ใช้ยา/iu,"CHEMICAL_REVIEW"]],classes=mappings.filter(([pattern])=>pattern.test(input.message)).map(([,optionClass])=>optionClass),selected=input.management_option_id?pkg.management.all_option_evaluations.find((item)=>item.management_option_id===input.management_option_id):classes.length===1?pkg.management.all_option_evaluations.find((item)=>item.option_class===classes[0]):null;
      if(!selected)return {action:"ASK_ONE_CLARIFICATION",response_type:"CLARIFICATION",text:"ต้องการเลือกทางเลือกใดจากรายการ F1 ที่แสดงอยู่ครับ",question:"ต้องการเลือกทางเลือกใดจากรายการ F1 ที่แสดงอยู่ครับ",ui_action:"OPEN_CASE",record:null};
      if(!["SUPPORTED_FOR_REVIEW","HUMAN_REVIEW_REQUIRED"].includes(selected.eligibility_state))return {action:"PRESENT_MANAGEMENT_REVIEW",response_type:"MANAGEMENT_REVIEW_SUMMARY",text:"ทางเลือกนี้ยังยืนยันไม่ได้จากสถานะ F1 ปัจจุบันครับ กรุณาดูเหตุผลและข้อมูลที่ยังขาดในหน้าเคส",question:null,ui_action:"OPEN_CASE",record:null};
      const decisionType={CONTINUE_MONITORING:"CONTINUE_MONITORING",EXPERT_REVIEW:"REQUEST_EXPERT_REVIEW",NO_ACTION_CURRENTLY_JUSTIFIED:"NO_ACTION_CURRENTLY"}[selected.option_class]??"SELECT_MANAGEMENT_OPTION",current=memory?.current_decision??null,decision=this.decisionActionOutcomeService.createDecision(userId,{request_id:`${input.request_id}-f2-decision`,...scope,management_review_id:pkg.management.management_review_id,decision_type:decisionType,selected_option_id:selected.management_option_id,selection_source:"STEP_E_EXPLICIT_TEXT",selection_raw_text:input.message,selection_source_span:{start:0,end:input.message.length,text:input.message},explicit_confirmation:true,expected_current_decision_id:current?.decision_id??null,supersedes_decision_id:current?.decision_id??null});
      return {action:"CONFIRM_MANAGEMENT_DECISION",response_type:"DECISION_CONFIRMED",text:decision.decision_state==="REVIEW_REQUIRED"?"บันทึกคำขอทบทวนโดยผู้เชี่ยวชาญแล้วครับ สถานะยังเป็นต้องทบทวน ไม่ใช่การอนุมัติให้ดำเนินการ":"บันทึกการตัดสินใจที่คุณระบุชัดเจนแล้วครับ การตัดสินใจนี้ยังไม่ใช่การยืนยันว่าลงมือทำแล้ว",question:null,ui_action:"OPEN_CASE",record:{decision_id:decision.decision_id}};
    }
    if(interpretation.intent==="ACTION_CONFIRMATION"){
      if(!input.performed_at)return {action:"ASK_ONE_CLARIFICATION",response_type:"CLARIFICATION",text:"ดำเนินการเมื่อเวลาใดครับ กรุณาระบุวันและเวลาเพื่อไม่ให้ระบบเดา",question:"ดำเนินการเมื่อเวลาใดครับ",ui_action:"OPEN_CASE",record:null};
      const actions=memory?.current_management_actions??[],planned=input.management_action_id?actions.find((item)=>item.management_action_id===input.management_action_id&&item.status==="PLANNED"):actions.filter((item)=>item.status==="PLANNED").length===1?actions.find((item)=>item.status==="PLANNED"):null,decision=memory?.current_decision;
      if(!planned||!decision||decision.decision_state!=="CONFIRMED")return {action:"ASK_ONE_CLARIFICATION",response_type:"CLARIFICATION",text:"ยังระบุแผนที่ดำเนินการไม่ชัดเจนครับ กรุณาเลือกแผนที่บันทึกไว้หนึ่งรายการ",question:"ดำเนินการตามแผนใดครับ",ui_action:"OPEN_CASE",record:null};
      const action=this.decisionActionOutcomeService.createAction(userId,{request_id:`${input.request_id}-f2-action`,...scope,decision_id:input.decision_id??decision.decision_id,action_kind:"CONFIRM_PERFORMED",action_source:"HUMAN_EXECUTION_CONFIRMATION",actor_source:"AUTHENTICATED_USER",performed_at:input.performed_at,explicit_confirmation:true,expected_current_action_id:planned.management_action_id,supersedes_action_id:planned.management_action_id,management_event_type:"UNKNOWN_MANAGEMENT_EVENT"});
      return {action:"RECORD_MANAGEMENT_ACTION",response_type:"ACTION_RECORDED",text:"บันทึกตามคำยืนยันของคุณว่าได้ดำเนินการแล้วครับ ข้อมูลนี้เป็นข้อเท็จจริงที่ผู้ใช้ยืนยัน ไม่ใช่คำแนะนำหรือผลลัพธ์",question:null,ui_action:"OPEN_CASE",record:{management_action_id:action.management_action_id}};
    }
    if(interpretation.intent==="OUTCOME_REPORT"){
      const performed=(memory?.current_management_actions??[]).filter((item)=>item.status==="PERFORMED"),action=input.management_action_id?performed.find((item)=>item.management_action_id===input.management_action_id):performed.length===1?performed[0]:null,decision=memory?.current_decision,state=/ดีขึ้น|น้อยลง|ลดลง/iu.test(input.message)?"DECREASE_OBSERVED":/มากขึ้น|เพิ่มขึ้น|แย่ลง/iu.test(input.message)?"INCREASE_OBSERVED":/เท่าเดิม|ไม่เปลี่ยน/iu.test(input.message)?"NO_CLEAR_CHANGE":null;
      if(!action||!decision||!state)return {action:"ASK_ONE_CLARIFICATION",response_type:"CLARIFICATION",text:"ต้องการผูกผลที่รายงานกับการดำเนินการใด และเป็นลดลง เพิ่มขึ้น หรือไม่เห็นการเปลี่ยนชัดเจนครับ",question:"ผลนี้ผูกกับการดำเนินการใดครับ",ui_action:"OPEN_CASE",record:null};
      const history=this.decisionActionOutcomeService.comparisonHistory(userId,action.management_action_id),result=this.decisionActionOutcomeService.createOutcomeComparison(userId,{request_id:`${input.request_id}-f2-outcome`,...scope,decision_id:input.decision_id??decision.decision_id,management_action_id:action.management_action_id,comparison_state:state,statement:input.message,source:"AUTHENTICATED_USER",limitations:["เป็นการเปรียบเทียบเชิงพรรณนาจากถ้อยคำที่ผู้ใช้ระบุ ไม่ใช่ข้อสรุปประสิทธิผลหรือเหตุและผล"],explicit_submission:true,expected_comparison_revision:history.length});
      return {action:"RECORD_OUTCOME",response_type:"OUTCOME_RECORDED",text:"บันทึกการเปรียบเทียบผลตามถ้อยคำของคุณแล้วครับ ระบบยังไม่สรุปประสิทธิผล เหตุและผล หรือการดื้อสารจากข้อความนี้",question:null,ui_action:"OPEN_CASE",record:{outcome_comparison_id:result.comparison.outcome_comparison_id}};
    }
    return null;
  }
  primaryResponse(interpretation,pkg,visual,message="") {
    if(interpretation.intent==="CANNOT_ANSWER")return {action:"ACK",response_type:"ACKNOWLEDGEMENT",text:"รับทราบครับ เก็บเรื่องนี้เป็นข้อมูลที่ยังไม่ทราบไว้ก่อน คุณกลับมาเพิ่มข้อความหรือรูปภายหลังได้",question:null,ui_action:"OPEN_CASE"};
    if(interpretation.intent==="USER_CORRECTION")return {action:"RECORD_EXPLICIT_FACTS",response_type:"ACKNOWLEDGEMENT",text:"รับทราบการแก้ไขครับ ข้อความเดิมยังอยู่ในประวัติ และระบบใช้ข้อมูลที่แก้ไขเป็นรายการปัจจุบัน",question:null,ui_action:"OPEN_CASE"};
    if(this.fieldCaptureAlpha&&["MANAGEMENT_QUERY","MANAGEMENT_SELECTION","ACTION_CONFIRMATION","OUTCOME_REPORT"].includes(interpretation.intent))return {action:"ACK",response_type:"SYSTEM_LIMITATION",text:"โหมดบันทึกภาคสนามนี้ช่วยเก็บสิ่งที่พบและข้อมูลที่ยังขาด แต่ไม่แนะนำสาร ผลิตภัณฑ์ อัตราใช้ หรือวิธีจัดการครับ",question:null,ui_action:"OPEN_CASE"};
    if(interpretation.intent==="MANAGEMENT_QUERY"){
      const review=pkg.management;
      if(!review)return {action:"PRESENT_MANAGEMENT_REVIEW",response_type:"MANAGEMENT_REVIEW_SUMMARY",text:"ตอนนี้ยังไม่มีเคสที่มีหลักฐานเพียงพอให้เปิดทบทวนทางเลือกการจัดการครับ",question:null,ui_action:"OPEN_FIELD"};
      const need=review.need_for_action.state,chemical=review.all_option_evaluations.find((item)=>item.option_class==="CHEMICAL_REVIEW"),chemicalAsked=/ยา|สารเคมี|พ่น|ใช้สาร|โดส|อัตรา/iu.test(message);
      if(need==="MORE_EVIDENCE_REQUIRED")return {action:"PRESENT_MANAGEMENT_REVIEW",response_type:"MANAGEMENT_REVIEW_SUMMARY",text:"ตอนนี้ข้อมูลยังไม่พอเปิดทางเลือกการจัดการครับ ขั้นถัดไปยังเป็นการเก็บหลักฐานตามคำแนะนำการตรวจที่กำกับไว้",question:null,ui_action:"OPEN_GUIDANCE"};
      if(chemicalAsked&&chemical.eligibility_state==="BLOCKED_BY_AUTHORITY")return {action:"PRESENT_MANAGEMENT_REVIEW",response_type:"MANAGEMENT_REVIEW_SUMMARY",text:"ตอนนี้ยังไม่ควรเปิดทางเลือกสารครับ เพราะข้อมูลอำนาจการใช้กับข้าวและปัญหานี้ยังไม่ครบ",question:null,ui_action:"OPEN_CASE"};
      if(chemicalAsked&&chemical.eligibility_state==="SUPPORTED_FOR_REVIEW")return {action:"PRESENT_MANAGEMENT_REVIEW",response_type:"MANAGEMENT_REVIEW_SUMMARY",text:"ตอนนี้เปิดการทบทวนทางสารเคมีในขั้นกำกับถัดไปได้แล้วครับ ขั้นนี้ยังไม่เลือกสาร ผลิตภัณฑ์ หรืออัตราใช้",question:null,ui_action:"OPEN_CASE"};
      const labels={CONTINUE_MONITORING:"ติดตามต่อ",CULTURAL_MANAGEMENT:"การจัดการเชิงเขตกรรม",MECHANICAL_MANAGEMENT:"การจัดการเชิงกล",BIOLOGICAL_MANAGEMENT:"การจัดการชีวภาพ",CHEMICAL_REVIEW:"ทบทวนทางสารเคมี",EXPERT_REVIEW:"ส่งผู้เชี่ยวชาญทบทวน",NO_ACTION_CURRENTLY_JUSTIFIED:"ยังไม่ดำเนินการในตอนนี้"},paths=review.user_facing_review_options.map((item)=>labels[item.option_class]).join(" กับ ");
      const blocked=chemical.eligibility_state==="BLOCKED_BY_AUTHORITY"?" ส่วนทางสารเคมียังเปิดไม่ได้เพราะข้อมูลอำนาจการใช้ยังไม่ครบ":"";
      return {action:"PRESENT_MANAGEMENT_REVIEW",response_type:"MANAGEMENT_REVIEW_SUMMARY",text:paths?`ตอนนี้ข้อมูลพอให้พิจารณาทางเลือกแล้วครับ มีทางที่ระบบรองรับให้ทบทวน: ${paths}${blocked}`:"ตอนนี้ยังไม่มีทางเลือกที่หลักฐานรองรับให้เปิดทบทวนครับ",question:null,ui_action:"OPEN_CASE"};
    }
    ...
    if(visual){if(visual.status!=="COMPLETED")return {action:"ACK",response_type:"SYSTEM_LIMITATION",text:naturalVisualReceipt(),question:null,ui_action:"OPEN_VISUAL_REVIEW"};const next=visual.result?.next_visual_action;if(next?.action==="BETTER_VIEW_REQUIRED")return {action:"REQUEST_VISUAL_EVIDENCE",response_type:"VISUAL_GUIDANCE",text:"รับภาพไว้แล้วครับ แต่จุดที่ต้องดูยังไม่ชัด ถ้าสะดวกส่งเพิ่มอีกหนึ่งมุมได้ครับ",question:null,ui_action:"OPEN_CAMERA"};return {action:"CHECK_VISUAL_EVIDENCE",response_type:"VISUAL_GUIDANCE",text:naturalVisualReceipt({automaticReadingAvailable:true}),question:null,ui_action:"OPEN_VISUAL_REVIEW"};}
    ...
    if(guidance){const visualGuidance=guidance.inspection_domain==="VISUAL_EVIDENCE",presentation=this.fieldCaptureAlpha?composeNaturalFieldGuidance({explicitFacts:interpretation.explicit_facts,guidance}):null;return {action:visualGuidance?"REQUEST_VISUAL_EVIDENCE":"REQUEST_FIELD_CHECK",response_type:visualGuidance?"VISUAL_GUIDANCE":"FIELD_GUIDANCE",text:presentation?.text??`รับทราบครับ ขั้นถัดไปที่ระบบแนะนำคือ ${guidance.title}: ${guidance.what_to_inspect}`,question:presentation?.question??null,ui_action:visualGuidance?"OPEN_CAMERA":"OPEN_GUIDANCE"};}
  ...
  unscopedTurn(userId,input,interpretation,turnId,providerInputFailure=null) {
  ...
  async executeTurn(userId,rawInput) {
    identifier(userId,"user_id");let input=validateInput(rawInput);const existing=this.db.prepare("SELECT response_json FROM governed_conversation_turns WHERE owner_user_id=? AND request_id=?").get(userId,input.request_id);if(existing)return {...JSON.parse(existing.response_json),idempotent_replay:true};
    const turnId=input.turn_id??`turn-${hash({userId,requestId:input.request_id}).slice(0,24)}`;if(input.conversation_id&&input.expected_conversation_revision!=null&&this.conversation(userId,input.conversation_id).revision!==input.expected_conversation_revision)fail("conversation changed before turn","VERSION_CONFLICT");
    let interpretation=localInterpretation(input.message,input),providerManifest=this.provider.getManifest(),providerInputFailure=null;
    if(interpretation.intent==="USER_CORRECTION"&&!input.corrects_turn_id&&input.conversation_id){const latest=this.db.prepare("SELECT turn_id FROM governed_conversation_turns WHERE owner_user_id=? AND conversation_id=? ORDER BY created_at DESC,turn_id DESC LIMIT 1").get(userId,input.conversation_id);if(latest)input={...input,corrects_turn_id:latest.turn_id};}
    if(input.corrects_turn_id){const corrected=this.db.prepare("SELECT turn_id,conversation_id FROM governed_conversation_turns WHERE turn_id=? AND owner_user_id=?").get(input.corrects_turn_id,userId);if(!corrected||input.conversation_id&&corrected.conversation_id!==input.conversation_id)fail("corrected turn scope not found","AUTHORIZATION_ERROR");}
    if(providerManifest.required){const generalUnscoped=input.entry_point==="GENERAL_CHAT"&&(input.remain_general||!input.field_id&&!input.route_context?.field_id&&!input.explicit_reference?.field_id&&!input.conversation_id),preliminary=generalUnscoped?{resolved:false}:this.resolveContext(userId,input,interpretation),providerPackage=preliminary.resolved?this.contextPackage(userId,preliminary,input):null,minimum=providerPackage?{...this.minimumProviderContext(providerPackage),entry_point:input.entry_point}:{entry_point:input.entry_point};try{const proposal=validateProviderProposal(await this.provider.interpret({message:input.message,minimumContext:minimum}),input.message),mergedFacts=[...interpretation.explicit_facts];for(const fact of proposal.explicit_facts)if(!mergedFacts.some((item)=>item.dimension===fact.dimension&&item.governed_code===fact.governed_code&&item.source_span.start===fact.source_span.start))mergedFacts.push(fact);const localPriority=["FOLLOW_UP_SCHEDULING","REMINDER_ACTION","MANAGEMENT_QUERY","MANAGEMENT_SELECTION","ACTION_CONFIRMATION","OUTCOME_REPORT","VISUAL_CHECK","FIELD_OBSERVATION","USER_HYPOTHESIS"].includes(interpretation.intent);interpretation={...interpretation,...proposal,intent:localPriority?interpretation.intent:proposal.intent,explicit_facts:mergedFacts,user_hypothesis:interpretation.user_hypothesis??proposal.user_hypothesis,interpretation_version:"provider-validated-conversation-interpretation/v1"};}catch(error){providerInputFailure=error?.category??"CONVERSATION_UNAVAILABLE";}}
    ...
    if(input.entry_point==="GENERAL_CHAT"&&input.remain_general)return this.unscopedTurn(userId,input,interpretation,turnId,providerInputFailure);
    if(input.entry_point==="GENERAL_CHAT"&&!input.field_id&&!input.route_context?.field_id&&!input.explicit_reference?.field_id&&!input.conversation_id&&["KNOWLEDGE_QUERY","NAVIGATION_REQUEST","SOCIAL_OR_OTHER"].includes(interpretation.intent))return this.unscopedTurn(userId,input,interpretation,turnId,providerInputFailure);
    ...
    const correctedFactRecordReferences=this.supersedeCorrectedFacts(userId,input,turnId),captured=this.capture(userId,resolution,interpretation,turnId,input.message),context={...resolution,case_id:captured.case_id},contextConflict=this.recordContextConflict(userId,context,turnId,input.message);let pkg=this.contextPackage(userId,context,input),visual=null;if(input.image_evidence_id&&interpretation.intent==="VISUAL_CHECK")visual=await this.visualPerceptionService.request(userId,{image_evidence_id:input.image_evidence_id,requested_target:input.requested_visual_target});
    const highStakesFallback=Boolean(providerInputFailure&&["MANAGEMENT_SELECTION","ACTION_CONFIRMATION","OUTCOME_REPORT","FOLLOW_UP_SCHEDULING","REMINDER_ACTION"].includes(interpretation.intent));
    const stepG=highStakesFallback?null:this.gResponse(userId,input,interpretation,pkg),f2=highStakesFallback||this.fieldCaptureAlpha||stepG?null:this.f2Response(userId,input,interpretation,pkg);if(stepG?.record||f2?.record)pkg=this.contextPackage(userId,context,input);const primary=highStakesFallback?{action:"ACK",response_type:"SYSTEM_LIMITATION",text:"บริการตีความไม่พร้อมครับ ข้อมูลแปลงและหลักฐานยังบันทึกได้ แต่การตัดสินใจหรือกำหนดเวลาต้องใช้แบบฟอร์มที่ยืนยันโดยผู้ใช้",question:null,ui_action:"OPEN_CASE"}:contextConflict?{action:"ASK_ONE_CLARIFICATION",response_type:"CLARIFICATION",text:"ข้อมูลระยะข้าวที่บันทึกไว้ต่างจากที่คุณเพิ่งบอกครับ ขอให้ยืนยันระยะปัจจุบันก่อน โดยระบบยังไม่เปลี่ยนข้อมูลแปลงให้อัตโนมัติ ตอนนี้ระยะข้าวเป็นอย่างที่คุณเพิ่งบอกใช่ไหมครับ?",question:"ตอนนี้ระยะข้าวเป็นอย่างที่คุณเพิ่งบอกใช่ไหมครับ?",ui_action:"OPEN_FIELD"}:stepG??f2??this.primaryResponse(interpretation,pkg,visual,input.message),now=this.now(),conversationId=input.conversation_id??`conversation-${this.idProvider()}`;
    let finalText=primary.text,compositionState=providerInputFailure?"DETERMINISTIC_INPUT_FALLBACK":"DETERMINISTIC_RESPONSE_FALLBACK";
    if(providerManifest.required&&!providerInputFailure){const plan={response_type:primary.response_type,governed_action:primary.action,required_text:primary.text,required_question:primary.question,one_action_only:true,scientific_authority:false};try{const guarded=guardComposedResponse(await this.provider.compose({responsePlan:plan}),plan);finalText=guarded.text;compositionState=guarded.composition_state;}catch{compositionState="DETERMINISTIC_RESPONSE_FALLBACK";}}
    ...
    const response={turn_id:turnId,conversation_id:conversationId,conversation_revision:prior?prior.revision+1:1,context:{resolved:true,field_id:context.field_id,season_id:context.season_id,case_id:context.case_id,resolution_source:context.resolution_source,context_hash:pkg.context_hash,context_version:CONVERSATION_CONTEXT_VERSION},intent:interpretation.intent,explicit_facts:interpretation.explicit_facts,user_hypothesis:interpretation.user_hypothesis,fact_record_references:captured.fact_record_references,corrected_fact_record_references:correctedFactRecordReferences,context_conflict:contextConflict,hypothesis_reference:captured.hypothesis_reference,execution_actions:captured.execution_actions,actions:[{action:primary.action}],evidence_action:currentEvidenceAction(primary,pkg),decision_action_outcome_reference:f2?.record??null,follow_up_reminder_reference:stepG?.record??null,assessment_reference:pkg.assessment?{assessment_id:pkg.assessment.assessment_id,revision:pkg.assessment.revision,source_bundle_hash:pkg.assessment.source_bundle_hash}:null,guidance_reference:pkg.guidance?{guidance_id:pkg.guidance.guidance_id,status:pkg.guidance.status,context_hash:pkg.guidance.context_hash}:null,management_review_reference:this.fieldCaptureAlpha?null:pkg.management?{management_review_id:pkg.management.management_review_id,context_hash:pkg.management.context_hash,need_for_action_state:pkg.management.need_for_action.state}:null,visual_references:compact([input.image_evidence_id,visual?.request?.analysis_request_id,visual?.result?.perception_result_id]),response_type:primary.response_type,text:finalText,question:primary.question,ui_action:primary.ui_action,composition_state:compositionState,provider_input_state:providerInputFailure?"DETERMINISTIC_FALLBACK":"VALIDATED_OR_NOT_REQUIRED",technical_detail:!this.fieldCaptureAlpha&&input.audience==="SPA"?{governed_codes:interpretation.explicit_facts.map((item)=>item.governed_code),context_package:pkg,execution_actions:captured.execution_actions}:null,limitations:this.fieldCaptureAlpha?["ข้อความดิบและข้อมูลที่บันทึกถูกเก็บแยกกัน","โหมดนี้ไม่ให้คำแนะนำสาร ผลิตภัณฑ์ อัตราใช้ หรือข้อสรุปวินิจฉัย"]:["บทสนทนาไม่ใช่หลักฐานจนกว่าข้อเท็จจริงชัดเจนจะผ่านการบันทึกแบบ governed","ผล Step C, Step D, Step F1, Step F2 และ Step G เป็นอำนาจของบริการเดิม ไม่ได้คำนวณโดยโมเดลสนทนา"],authority:{conversation:"SERVER_ORCHESTRATED",memory:"SERVER_PERSISTED_NON_EVIDENCE",scientific:this.fieldCaptureAlpha?"GOVERNED_INVESTIGATION_AND_GUIDANCE":"INVESTIGATION_BACKBONE_STEP_C_STEP_D_STEP_F1_STEP_F2_STEP_G",provider_scientific_authority:false},provider:providerManifest,created_at:now,output_version:CONVERSATION_OUTPUT_VERSION};
