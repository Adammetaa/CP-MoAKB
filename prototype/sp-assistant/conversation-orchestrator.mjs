...
export const CONVERSATION_ENUMS = Object.freeze({
  ...
  intents: ["FIELD_OBSERVATION", "FIELD_STATUS_UPDATE", "QUESTION", "VISUAL_CHECK", "GUIDANCE_RESPONSE", "CASE_FOLLOW_UP", "CONTEXT_SELECTION", "USER_HYPOTHESIS", "MANAGEMENT_QUERY", "MANAGEMENT_SELECTION", "ACTION_CONFIRMATION", "OUTCOME_REPORT", "KNOWLEDGE_QUERY", "NAVIGATION_REQUEST", "SOCIAL_OR_OTHER"],
  extractionStatuses: ["EXPLICIT", "AMBIGUOUS", "UNSUPPORTED_INFERENCE"],
  responseTypes: ["ACKNOWLEDGEMENT", "CLARIFICATION", "FIELD_GUIDANCE", "VISUAL_GUIDANCE", "INVESTIGATION_SUMMARY", "UNCERTAINTY_SUMMARY", "EXPERT_HANDOFF", "LAB_HANDOFF", "NO_ADDITIONAL_ACTION", "MANAGEMENT_REVIEW_SUMMARY", "DECISION_CONFIRMED", "ACTION_RECORDED", "OUTCOME_RECORDED", "GENERAL_KNOWLEDGE", "SYSTEM_LIMITATION", "ERROR_SAFE"],
  uiActions: ["NONE", "OPEN_FIELD", "OPEN_CASE", "OPEN_CAMERA", "OPEN_GUIDANCE", "OPEN_VISUAL_REVIEW", "OPEN_KNOWLEDGE"],
  governedActions: ["ACK", "RECORD_EXPLICIT_FACTS", "ASK_ONE_CLARIFICATION", "PRESENT_CURRENT_GUIDANCE", "REQUEST_FIELD_CHECK", "REQUEST_VISUAL_EVIDENCE", "CHECK_VISUAL_EVIDENCE", "PRESENT_INVESTIGATION_SUMMARY", "PRESENT_UNCERTAINTY_SUMMARY", "REQUEST_HUMAN_REVIEW", "REQUEST_EXPERT_REVIEW", "REQUEST_LAB_EVIDENCE", "NO_ADDITIONAL_ACTION", "NAVIGATE", "KNOWLEDGE_LOOKUP", "PRESENT_MANAGEMENT_REVIEW", "CONFIRM_MANAGEMENT_DECISION", "RECORD_MANAGEMENT_ACTION", "RECORD_OUTCOME"],
...
function localInterpretation(message, input) {
  ...
  let intent = /ทำแล้ว|ดำเนินการแล้ว|ลงมือแล้ว/iu.test(message) ? "ACTION_CONFIRMATION" : /ดีขึ้น|น้อยลง|ลดลง|มากขึ้น|เพิ่มขึ้น|แย่ลง|เท่าเดิม|ไม่เปลี่ยน/iu.test(message) ? "OUTCOME_REPORT" : /(?:เลือก|เอา)(?:ทาง|ตัวเลือก)|ติดตามต่อ|ขอผู้เชี่ยวชาญ|ยังไม่ทำตอนนี้/iu.test(message) ? "MANAGEMENT_SELECTION" : /ยา|สารเคมี|พ่น|กำจัด|ใช้สาร|โดส|อัตรา|ทางเลือก|จัดการ|ทำอะไรได้บ้าง/iu.test(message) ? "MANAGEMENT_QUERY" : input.image_evidence_id || /ถ่ายแบบนี้|ดูรูป|ดูภาพ|รูปนี้|ภาพนี้/iu.test(message) ? "VISUAL_CHECK" : facts.length ? "FIELD_OBSERVATION" : userHypothesis ? "USER_HYPOTHESIS" : /ความรู้|คืออะไร|ทั่วไป/iu.test(message) ? "KNOWLEDGE_QUERY" : /เปิด|ไปหน้า/iu.test(message) ? "NAVIGATION_REQUEST" : /ขอบคุณ|สวัสดี/iu.test(message) ? "SOCIAL_OR_OTHER" : "QUESTION";
  return { interpretation_version:"server-governed-thai-interpretation/v1", intent, server_explicit_f2_intent:["MANAGEMENT_SELECTION","ACTION_CONFIRMATION","OUTCOME_REPORT"].includes(intent), explicit_facts:facts, user_hypothesis:userHypothesis, unsupported_inferences:[], raw_message_is_evidence:false };
}
...
export class GovernedConversationOrchestrator {
  constructor(db, { backbone, captureAdapter = null, assessmentService, guidanceService, visualEvidenceService, visualPerceptionService, managementService, decisionActionOutcomeService = null, provider = new ConversationProvider(), clock = () => new Date(), idProvider = () => randomUUID() } = {}) {
    this.db=db; this.backbone=backbone; this.captureAdapter=captureAdapter??new GovernedConversationCaptureAdapter(backbone); this.assessmentService=assessmentService; this.guidanceService=guidanceService; this.visualEvidenceService=visualEvidenceService; this.visualPerceptionService=visualPerceptionService; this.managementService=managementService; this.decisionActionOutcomeService=decisionActionOutcomeService; this.provider=provider; this.clock=clock; this.idProvider=idProvider; this.inFlight=new Map();
  }
  ...
  contextPackage(userId,context,input) {
    ...
    let assessment=null,guidance=null,management=null,decisionActionOutcome=null;if(context.case_id){assessment=this.assessmentService.assess(userId,{field_id:context.field_id,season_id:context.season_id,case_id:context.case_id});guidance=this.guidanceService.current(userId,{field_id:context.field_id,season_id:context.season_id,case_id:context.case_id});management=this.managementService.current(userId,{field_id:context.field_id,season_id:context.season_id,case_id:context.case_id});decisionActionOutcome=this.decisionActionOutcomeService?.context(userId,{field_id:context.field_id,season_id:context.season_id,case_id:context.case_id})??null;}
  ...
  async executeTurn(userId,rawInput) {
    ...
    const f2=this.f2Response(userId,input,interpretation,pkg);if(f2?.record)pkg=this.contextPackage(userId,context,input);const primary=f2??this.primaryResponse(interpretation,pkg,visual,input.message),now=this.now(),conversationId=input.conversation_id??`conversation-${this.idProvider()}`;
