...
export const CONVERSATION_ENUMS = Object.freeze({
  ...
  responseTypes: ["ACKNOWLEDGEMENT", "CLARIFICATION", "FIELD_GUIDANCE", "VISUAL_GUIDANCE", "INVESTIGATION_SUMMARY", "UNCERTAINTY_SUMMARY", "EXPERT_HANDOFF", "LAB_HANDOFF", "NO_ADDITIONAL_ACTION", "MANAGEMENT_REVIEW_SUMMARY", "GENERAL_KNOWLEDGE", "SYSTEM_LIMITATION", "ERROR_SAFE"],
...
export class GovernedConversationOrchestrator {
  constructor(db, { backbone, captureAdapter = null, assessmentService, guidanceService, visualEvidenceService, visualPerceptionService, managementService, provider = new ConversationProvider(), clock = () => new Date(), idProvider = () => randomUUID() } = {}) {
    this.db=db; this.backbone=backbone; this.captureAdapter=captureAdapter??new GovernedConversationCaptureAdapter(backbone); this.assessmentService=assessmentService; this.guidanceService=guidanceService; this.visualEvidenceService=visualEvidenceService; this.visualPerceptionService=visualPerceptionService; this.managementService=managementService; this.provider=provider; this.clock=clock; this.idProvider=idProvider; this.inFlight=new Map();
  }
  ...
  contextPackage(userId,context,input) {
    ...
    let assessment=null,guidance=null,management=null;if(context.case_id){assessment=this.assessmentService.assess(userId,{field_id:context.field_id,season_id:context.season_id,case_id:context.case_id});guidance=this.guidanceService.current(userId,{field_id:context.field_id,season_id:context.season_id,case_id:context.case_id});management=this.managementService.current(userId,{field_id:context.field_id,season_id:context.season_id,case_id:context.case_id});}
  ...
  async executeTurn(userId,rawInput) {
    ...
    const primary=this.primaryResponse(interpretation,pkg,visual,input.message),now=this.now(),conversationId=input.conversation_id??`conversation-${this.idProvider()}`;
