...
for (const required of ["name=\"login_id\"","capture=\"environment\" data-${surface}-camera","📷 ถ่ายภาพ","🖼 เลือกรูป","📎 แนบไฟล์","governedRuntime.captureStatement(text","serverWorkspace.logout()","captureOnly()"] ) if (!fieldApp.includes(required)) failures.push(`field-app.js missing mobile capture workflow: ${required}`);
for (const required of ["runRecoverableChatSubmission", "chatRetryText=text", "ยังส่งไม่สำเร็จ", "VISUAL_EVIDENCE_CAPTURE"]) if (!fieldApp.includes(required)) failures.push(`field-app.js missing M0A.1 recovery or natural image contract: ${required}`);
