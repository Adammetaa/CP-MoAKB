...
const root=await mkdtemp(join(tmpdir(),"cpmoakb-rfv1-ui-")),dbPath=join(root,"pilot.sqlite"),exportDir=join(root,"exports"),uploadDir=join(root,"uploads"),fixtureBytes=Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Y9ZlSAAAAAASUVORK5CYII=","base64"),fixtureOne=join(root,"uat-evidence-1.png"),fixtureTwo=join(root,"หลักฐานภาพมุมกว้างของแปลง-ชื่อไฟล์ยาวสำหรับทดสอบ-responsive-layout-2.png");
await writeFile(fixtureOne,fixtureBytes);await writeFile(fixtureTwo,Buffer.concat([fixtureBytes,Buffer.from([0])]));
const seed=await new PilotStore({dbPath,exportDir,pilotProfile:"FIELD_CAPTURE_ALPHA",scopedIdentities:users,investigationCandidateProvider:createEmptyCandidateProvider()}).open();
...
seed.close();
const server=await startServer({port:Number(process.env.PORT??4173),host:"127.0.0.1",dbPath,exportDir,uploadDir,pilotUsers:users,pilotProfile:"FIELD_CAPTURE_ALPHA",investigationCandidateProvider:createEmptyCandidateProvider()});
console.log(JSON.stringify({url:`http://localhost:${server.address().port}/`,health:`http://localhost:${server.address().port}/health`,login_id:fieldUser.login_id,password:fieldUser.password,build:server.pilotRuntime.build,fixtures:[fixtureOne,fixtureTwo]}));
