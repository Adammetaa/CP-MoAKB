...
def state(chain: dict) -> str:
    return cast(
        str,
        run_js(
            f"window.SPDecisionAuthority.evaluateRegulatoryChain({json.dumps(chain)})"
        ),
    )
...
def test_bounded_eligibility_states_are_complete() -> None:
    assert set(run_js("window.SPDecisionAuthority.eligibilityStates")) == {
        "NO_REGULATORY_EVIDENCE",
        "REGISTRATION_IDENTITY_MATCH_ONLY",
        "REGULATORY_RELATIONSHIP_AMBIGUOUS",
        "REGISTRATION_STATUS_UNRESOLVED",
...
def test_identity_ambiguous_status_and_complete_chain_scenarios() -> None:
    assert (
        state({"officialEvidence": True, "identity": "A"})
        == "REGISTRATION_IDENTITY_MATCH_ONLY"
    )
    assert (
        state(
    ...
    joined = {
        ...
        "target": "T",
        "useContext": "U",
        "recordIdentifier": "R-1",
        "defensibleJoinKey": True,
    }
    assert state(joined) == "REGISTRATION_STATUS_UNRESOLVED"
    assert (
        state({**joined, "registrationStatus": "CURRENTLY_REGISTERED"})
        == "ELIGIBLE_FOR_DECISION_REVIEW"
    )
...
def test_mixture_remains_one_regulatory_identity() -> None:
    chain = {
        ...
        "target": "T",
        "useContext": "U",
        "recordIdentifier": "R-2",
        "defensibleJoinKey": True,
        "registrationStatus": "CURRENTLY_REGISTERED",
    }
...
def test_recovered_priority_evidence_remains_blocked_without_join_key() -> None:
    ...
    assert review["brown-planthopper"]["status"] == "HUMAN_REVIEW_REQUIRED"
    assert review["leaffolder"]["status"] == "HUMAN_REVIEW_REQUIRED"
    assert review["brown-planthopper"]["recordIdentifier"] is None
    assert review["brown-planthopper"]["defensibleJoinKey"] is False
...
def test_two_key_gate_stays_closed_for_ambiguous_production_evidence() -> None:
    ...
    assert result["management"]["registrationStatus"] == "HUMAN_REVIEW_REQUIRED"
    assert result["management"]["chemicalGate"] == "CHEMICAL_REVIEW_BLOCKED"
    assert result["management"]["chemicalRecommendation"] == "BLOCKED"


def test_sprint_077r_resolution_remains_partial_without_stable_join() -> None:
    resolution = run_js("window.SPDecisionAuthority.regulatoryResolution")
    assert resolution["strategy"] == "record-by-record human review"
    assert resolution["officialLabelsLocated"] == 0
    assert resolution["stableJoins"] == 0
    assert resolution["completeChains"] == 0
    assert resolution["currentEligibleChains"] == 0
    assert resolution["recommendationProduced"] is False
    assert (
        resolution["result"] == "PARTIAL — HUMAN REGULATORY RESOLUTION STILL REQUIRED"
    )
