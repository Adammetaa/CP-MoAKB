...
def test_sources_and_authorities_are_exact_and_separate() -> None:
    ...
    assert "UI-CPM-001/v1" in text and "UI-CPM-002/v1" in text
    assert "not silently converted to HRAC" in text
...
def test_management_options_are_evidence_backed_and_not_recommendations() -> None:
    ...
    assert "Management evidence required" in text
    assert "do not select, prioritize, decide, or recommend" in text
    assert "No disease Management Option is inferred" in text
